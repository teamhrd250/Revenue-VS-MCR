...
if not dept.empty:
    # Plotly scatter size cannot contain NaN, negative, or all-zero values.
    # Use a safe positive bubble size while keeping real revenue visible in hover/table.
    dept_plot = dept.copy()
    ...
    if dept_plot["Bubble_Size"].max() <= 0:
        ...
        dept_plot["Bubble_Size"] = dept_plot["Bubble_Size"].replace(0, min_positive * 0.10)
        bubble_note = "Bubble size memakai Dept Revenue; nilai nol diganti minimum visual agar chart tidak error."
    ...
    fig = px.scatter(
        ...
        size="Bubble_Size",
        color="FJA_Category",
        hover_data=["Dept_ID", "Dept_Name", "Function_Group", "Dept_Revenue", "Dept_RPE", "Revenue_per_Cost"],
        title="Department Cost vs Capacity vs Revenue",
    )
    fig.update_layout(margin=dict(l=10, r=10, t=45, b=10))
    st.plotly_chart(fig, use_container_width=True)
    st.caption(bubble_note)
