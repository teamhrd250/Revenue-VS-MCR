...
st.markdown("""
<style>
.block-container {padding-top: 1.15rem; padding-bottom: 1rem; max-width: 1500px;}
[data-testid="stSidebar"] {background: #f8fafc;}
.sep-header-wrap {display:flex; align-items:flex-start; justify-content:space-between; gap:24px; margin-bottom:10px;}
.sep-title {font-size:34px; line-height:1.05; font-weight:900; color:#172033; margin:0 0 8px 0;}
.sep-edition {font-size:15px; font-weight:650; color:#334155; margin-bottom:8px;}
.sep-subtitle {font-size:13px; color:#64748b;}
.sep-logo-box {display:flex; justify-content:flex-end; align-items:flex-start; padding-top:2px;}
.sep-feature-band {display:grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap:14px; padding:14px 18px; border:1px solid #e5e7eb; border-radius:14px; background:linear-gradient(180deg,#ffffff 0%,#f8fafc 100%); box-shadow:0 4px 18px rgba(15,23,42,.05); margin:12px 0 14px 0;}
.sep-feature {display:flex; gap:10px; align-items:flex-start;}
.sep-feature .ico {font-size:28px; line-height:1;}
.sep-feature .title {font-size:12px; font-weight:800; color:#172033; margin-bottom:4px;}
.sep-feature .desc {font-size:11px; color:#64748b; line-height:1.45;}
div[data-testid="stTabs"] button {font-size:12px; font-weight:700; color:#334155;}
div[data-testid="stTabs"] [aria-selected="true"] {color:#ef4444 !important;}
div[data-testid="stMetric"] {border:1px solid #e5e7eb; border-radius:14px; padding:14px 16px; box-shadow:0 4px 16px rgba(15,23,42,.03); background:#fff;}
.sep-section-title {font-size:18px; font-weight:900; color:#172033; margin:8px 0 12px 0;}
.sep-card-grid {display:grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap:14px; margin:8px 0 12px 0;}
.sep-kpi-card {border:1px solid #e5e7eb; border-radius:14px; background:#fff; padding:16px 18px; min-height:132px; box-shadow:0 4px 16px rgba(15,23,42,.035);}
.sep-kpi-label {font-size:12px; font-weight:800; color:#475569; margin-bottom:12px;}
.sep-kpi-value {font-size:30px; font-weight:800; color:#172033; line-height:1.05;}
.sep-kpi-delta {display:inline-block; margin-top:10px; padding:4px 8px; border-radius:999px; font-size:12px; font-weight:800;}
.sep-delta-pos {background:#dcfce7; color:#16a34a;}
.sep-delta-neg {background:#fee2e2; color:#ef4444;}
.sep-kpi-sub {font-size:11px; color:#64748b; margin-top:7px;}
.sep-status-big {font-size:24px; font-weight:900; margin:9px 0 8px 0;}
.sep-status-blue {color:#2563eb;}
.sep-status-amber {color:#d97706;}
.sep-memo {display:flex; gap:14px; align-items:flex-start; border:1px solid #dbeafe; border-radius:14px; background:linear-gradient(90deg,#eff6ff 0%,#f8fbff 100%); padding:16px 18px; color:#1e3a8a; line-height:1.65; font-size:14px;}
.sep-memo-icon {font-size:30px;}
.sep-health-panel {border:1px solid #e5e7eb; border-radius:16px; background:#fff; padding:18px 18px; box-shadow:0 4px 16px rgba(15,23,42,.035);}
.sep-health-row {display:grid; grid-template-columns: 118px 1fr 44px; gap:12px; align-items:center; margin:10px 0;}
.sep-health-label {font-size:12px; color:#475569; font-weight:700;}
.sep-bar-bg {height:9px; background:#e5e7eb; border-radius:999px; overflow:hidden;}
.sep-bar-fill {height:9px; border-radius:999px;}
.sep-score-badge {display:inline-block; padding:5px 10px; border-radius:999px; color:#fff; font-size:12px; font-weight:900; background:#22c55e;}
.sep-footer {font-size:12px; color:#64748b; display:flex; justify-content:space-between; border-top:1px solid #e5e7eb; padding-top:10px; margin-top:12px;}
@media (max-width: 1100px) {.sep-feature-band,.sep-card-grid{grid-template-columns:repeat(2,minmax(0,1fr));}.sep-header-wrap{display:block;}.sep-logo-box{justify-content:flex-start;margin-top:8px;}}
</style>
...
def render_header():
    ...
    c1, c2 = st.columns([7.5, 1.15])
    with c1:
        st.markdown("""
        <div class="sep-title">Starcom Executive Platform (SEP) V4</div>
        <div class="sep-edition">Executive Intelligence Edition</div>
        <div class="sep-subtitle">MCR, revenue leverage, workforce productivity, capacity risk, benchmark framework, and board decision.</div>
        """, unsafe_allow_html=True)
    with c2:
        if logo:
    ...
    <div class="sep-feature-band">
      <div class="sep-feature"><div class="ico">🎯</div><div><div class="title">Executive Decision Support</div><div class="desc">Insight yang relevan untuk pengambilan keputusan direksi dan management.</div></div></div>
      <div class="sep-feature"><div class="ico">📈</div><div><div class="title">Performance & Productivity</div><div class="desc">Monitoring kinerja revenue, MCR, margin, dan produktivitas karyawan.</div></div></div>
      <div class="sep-feature"><div class="ico">🛡️</div><div><div class="title">Risk & Capacity Control</div><div class="desc">Identifikasi risiko kapasitas, delivery, dan efisiensi organisasi.</div></div></div>
      <div class="sep-feature"><div class="ico">👥</div><div><div class="title">Benchmark & Best Practice</div><div class="desc">Framework benchmark berbasis industri Telekomunikasi & IT Integrator.</div></div></div>
      <div class="sep-feature"><div class="ico">📄</div><div><div class="title">Board Ready Report</div><div class="desc">Ringkasan eksekutif siap untuk board meeting dan stakeholder.</div></div></div>
    </div>
    """, unsafe_allow_html=True)
...
def _delta_class(x) -> str:
    try:
        if x is None or pd.isna(x):
            return "sep-delta-pos"
        return "sep-delta-pos" if float(x) >= 0 else "sep-delta-neg"
    except Exception:
        return "sep-delta-pos"


def kpi_card(label: str, value: str, delta: str = "", delta_raw=None, sub: str = "", icon: str = "") -> str:
    cls = _delta_class(delta_raw)
    delta_html = f'<div class="sep-kpi-delta {cls}">{delta}</div>' if delta else ""
    return f"""
    <div class="sep-kpi-card">
      <div class="sep-kpi-label">{icon} {label}</div>
      <div class="sep-kpi-value">{value}</div>
      {delta_html}
      <div class="sep-kpi-sub">{sub}</div>
    </div>
    """


def status_card(label: str, status: str, sub: str, css_class: str = "sep-status-blue", icon: str = "") -> str:
    return f"""
    <div class="sep-kpi-card">
      <div class="sep-kpi-label">{icon} {label}</div>
      <div class="sep-status-big {css_class}">{status}</div>
      <div class="sep-kpi-sub" style="font-size:13px;line-height:1.6;">{sub}</div>
    </div>
    """


def health_bar(label: str, value: float) -> str:
    try:
        val = max(0, min(100, float(value)))
    except Exception:
        val = 0
    color = "#22c55e" if val >= 85 else "#d9d90f" if val >= 70 else "#f59e0b" if val >= 55 else "#ef4444"
    return f"""
    <div class="sep-health-row">
      <div class="sep-health-label">{label}</div>
      <div class="sep-bar-bg"><div class="sep-bar-fill" style="width:{val:.0f}%; background:{color};"></div></div>
      <div style="font-size:12px;color:#64748b;text-align:right;font-weight:800;">{val:.0f}/100</div>
    </div>
    """


def health_rows_html(bs: Dict[str, object]) -> str:
    comps = bs.get("components", {}) if isinstance(bs, dict) else {}
    revenue = comps.get("Revenue Growth", 0)
    margin = comps.get("Margin Quality", 0)
    people = np.nanmean([comps.get("RPE Growth", np.nan), comps.get("MCR Health", np.nan)])
    capacity = comps.get("Capacity Risk", 0)
    risk = np.nanmean([margin, capacity])
    return "".join([
        health_bar("Revenue Health", revenue),
        health_bar("Margin Health", margin),
        health_bar("People Health", people),
        health_bar("Capacity Health", capacity),
        health_bar("Risk Health", risk),
    ])
...
# 1 Executive Summary
with boards[0]:
    st.markdown(f'<div class="sep-section-title">KPI Snapshot ({period["Period_Label"]}) <span style="color:#94a3b8;font-size:14px;">ⓘ</span></div>', unsafe_allow_html=True)
    if mode == "YTD Comparable" and data.get("monthly_ytd_used"):
        ...
        st.caption(data["mode_note"])

    left, right = st.columns([2.1, 0.95], gap="medium")
    with left:
        st.markdown("<div class='sep-card-grid'>", unsafe_allow_html=True)
        st.markdown(kpi_card("Revenue", _money(latest.get("Total_Revenue"), currency), _pct(latest.get("Revenue_YoY")), latest.get("Revenue_YoY"), f"vs {period['Period_Label'].replace(str(year_sel), str(year_sel-1))}", "💰"), unsafe_allow_html=True)
        st.markdown(kpi_card("MCR", _pct(latest.get("MCR_Pct")), _pp(latest.get("MCR_Delta")), -latest.get("MCR_Delta") if pd.notna(latest.get("MCR_Delta")) else np.nan, f"Status: {mh['state']}", "📊"), unsafe_allow_html=True)
        st.markdown(kpi_card("Gross Margin", _pct(latest.get("Gross_Margin_Pct")), _pp(latest.get("GrossMargin_Delta")), latest.get("GrossMargin_Delta"), f"Status: {mq['state']}", "📈"), unsafe_allow_html=True)
        st.markdown(kpi_card("RPE (Revenue/Employee)", _money(latest.get("RPE"), currency), _pct(latest.get("RPE_YoY")), latest.get("RPE_YoY"), f"vs {period['Period_Label'].replace(str(year_sel), str(year_sel-1))}", "⚡"), unsafe_allow_html=True)
        st.markdown(kpi_card("Headcount", _num(latest.get("Total_Headcount")), _pct(latest.get("Headcount_YoY")), latest.get("Headcount_YoY"), f"vs {period['Period_Label'].replace(str(year_sel), str(year_sel-1))}", "👤"), unsafe_allow_html=True)
        st.markdown(kpi_card("Manpower Cost", _money(latest.get("Total_Manpower_Cost"), currency), _pct(latest.get("ManpowerCost_YoY")), latest.get("ManpowerCost_YoY"), f"vs {period['Period_Label'].replace(str(year_sel), str(year_sel-1))}", "💼"), unsafe_allow_html=True)
        st.markdown(status_card("Business Posture", bp["state"], bp["hint"], "sep-status-blue", "🛡️"), unsafe_allow_html=True)
        st.markdown(status_card("Margin Quality", mq["state"], mq["hint"], "sep-status-amber" if mq["tone"] == "warning" else "sep-status-blue", "🏅"), unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div class='sep-section-title' style='margin-top:16px;'>Board Memo <span style='color:#94a3b8;font-size:14px;'>ⓘ</span></div>", unsafe_allow_html=True)
        st.markdown(f"""
        <div class="sep-memo">
          <div class="sep-memo-icon">📄</div>
          <div>
            Pada {period['Period_Label']}, Starcom mencatat Revenue <b>{_money(latest.get('Total_Revenue'), currency)}</b> ({_pct(latest.get('Revenue_YoY'))})
            dengan MCR <b>{_pct(latest.get('MCR_Pct'))}</b> ({mh['state'].replace('_',' ')}) dan RPE <b>{_money(latest.get('RPE'), currency)}</b> ({_pct(latest.get('RPE_YoY'))}).
            Business Posture berada di kategori <b>{bp['state']}</b>. Fokus utama adalah <b>{action_title}</b> untuk menjaga efisiensi manpower cost,
            meningkatkan gross margin, dan mengelola kapasitas untuk memastikan sustainable growth.
          </div>
        </div>
        """, unsafe_allow_html=True)

    with right:
        st.markdown("<div class='sep-health-panel'>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:18px;font-weight:900;color:#334155;margin-bottom:4px;'>Company Health Index <span style='color:#94a3b8;font-size:14px;'>ⓘ</span></div>", unsafe_allow_html=True)
        fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=bs["score"],
            number={"suffix": "\n/100", "font": {"size": 42, "color": "#16a34a" if bs["score"] >= 85 else "#f59e0b"}},
            gauge={
                "axis": {"range": [0, 100], "visible": False},
                "bar": {"color": "rgba(0,0,0,0)"},
                "steps": [
                    {"range": [0, 55], "color": "#ef4444"},
                    {"range": [55, 70], "color": "#f59e0b"},
                    {"range": [70, 85], "color": "#d9d90f"},
                    {"range": [85, 100], "color": "#22c55e"},
                ],
                "threshold": {"line": {"color": "#111827", "width": 4}, "thickness": 0.8, "value": bs["score"]},
            },
        ))
        fig.update_layout(height=265, margin=dict(l=10, r=10, t=10, b=10), paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
        st.markdown(f"<div style='text-align:center;margin-top:-28px;margin-bottom:18px;'><span class='sep-score-badge'>{'EXCELLENT' if bs['score'] >= 85 else bs['label'].upper()}</span></div>", unsafe_allow_html=True)
        st.markdown(health_rows_html(bs), unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown(f"<div class='sep-footer'><span>Periode Data: {period['Period_Label']} | Bulan Tertutup: {period['Months_Closed']} / 12 | Mode: {mode} | Confidence: {period['Data_Confidence']}</span><span>© 2026 Starcom Solusindo. All rights reserved.</span></div>", unsafe_allow_html=True)
