...
def clean_and_calc(sheets: Dict[str, pd.DataFrame], analysis_mode: str = "Actual") -> Dict[str, pd.DataFrame]:
    ...
    # YTD / Annualized logic for Board comparison.
    # Actual: read input as-is.
    # Annualized: scale revenue, COGS, project revenue/COGS, and manpower cost by 12 / Months_Closed.
    #   Because numerator and denominator are scaled with the same factor, MCR and Gross Margin % may stay the same.
    # YTD Comparable: convert all years to the same month basis as the latest YTD year.
    #   Example: if 2026 has 4 months closed, 2025 FY is proxied as 4/12 of FY 2025.
    # MCR Focus: scale revenue only to full-year run-rate, while manpower cost and COGS remain actual/YTD.
    #   This intentionally changes MCR and Gross Margin, and should be used as directional scenario reading.
    _cal_tmp = calendar.copy()
    _cal_tmp["Months_Closed"] = pd.to_numeric(_cal_tmp["Months_Closed"], errors="coerce").fillna(12).clip(lower=1, upper=12)
    _ytd_months = _cal_tmp.loc[_cal_tmp["Months_Closed"] < 12, "Months_Closed"]
    ...
    if analysis_mode in ["Annualized", "YTD Comparable", "MCR Focus"]:
        _rev_factor_map = annualization_map if analysis_mode in ["Annualized", "MCR Focus"] else ytd_comparable_factor_map
    ...
    if "REVENUE_BY_DEPT" in sheets and not sheets["REVENUE_BY_DEPT"].empty:
        ...
        if analysis_mode in ["Annualized", "YTD Comparable", "MCR Focus"]:
            _rd_factor_map = annualization_map if analysis_mode in ["Annualized", "MCR Focus"] else ytd_comparable_factor_map
    ...
    if "PROJECT_MARGIN" in sheets and not sheets["PROJECT_MARGIN"].empty:
        ...
        if analysis_mode in ["Annualized", "YTD Comparable", "MCR Focus"]:
            _pm_factor_map = annualization_map if analysis_mode in ["Annualized", "MCR Focus"] else ytd_comparable_factor_map
            _pm_factor = pm["Year"].astype("float").map(_pm_factor_map).fillna(1.0)
            if analysis_mode == "MCR Focus":
                if "Revenue" in pm.columns:
                    pm["Revenue"] = pm["Revenue"] * _pm_factor
            else:
        ...
        elif "Revenue" in pm.columns and "COGS" in pm.columns:
            pm["Gross_Profit"] = pm["Revenue"] - pm["COGS"]
...
with st.sidebar:
    ...
    analysis_mode = st.radio(
        "Compare mode",
        ["Actual", "Annualized", "YTD Comparable", "MCR Focus"],
        index=2 if ((_calendar_preview["Months_Closed"] < 12).any() if not _calendar_preview.empty else False) else 0,
        help="Actual = angka sesuai data. Annualized = semua amount disetahunkan. YTD Comparable = semua tahun disetarakan ke jumlah bulan YTD. MCR Focus = revenue run-rate, biaya tetap actual/YTD."
    )
...
if analysis_mode == "Actual" and period_info.get("Months_Closed", 12) < 12:
    ...
    st.info("Annualized mode aktif: Revenue, COGS, Dept Revenue, Project Revenue/COGS, dan Manpower Cost disetahunkan. Headcount tetap sebagai average capacity. Catatan: MCR dan Gross Margin % bisa tetap sama karena numerator dan denominator dikali faktor yang sama.")
elif analysis_mode == "YTD Comparable":
    st.info(f"YTD Comparable mode aktif: semua tahun dikonversi ke basis {data.get('comparable_months', period_info.get('Months_Closed'))} bulan. Ini paling apple-to-apple untuk membandingkan FY 2025 proxy vs YTD 2026.")
elif analysis_mode == "MCR Focus":
    st.warning("MCR Focus mode aktif: hanya revenue yang disetahunkan, sedangkan manpower cost dan COGS tetap actual/YTD. Mode ini sengaja membuat MCR dan Gross Margin berubah untuk membaca revenue run-rate terhadap biaya berjalan; gunakan sebagai directional scenario, bukan laporan akuntansi.")
...
with st.expander("How to Read Executive Summary"):
    ...
    - **Annualized** yang benar secara run-rate akan mengalikan revenue dan biaya dengan faktor yang sama, sehingga **MCR% dan Gross Margin% bisa tidak berubah**.
    - Gunakan **YTD Comparable** untuk membandingkan 2025 vs 2026 pada basis bulan yang sama.
    - Gunakan **MCR Focus** jika ingin melihat dampak revenue run-rate terhadap MCR dan Gross Margin dengan biaya actual/YTD.
    """)
