...
def board_priority_focus(board_status: str, cap_state: str, margin_state: str, mcr_state: str) -> str:
    if cap_state in ["HIGH", "CRITICAL"]:
        return "Capacity & Delivery Stabilization"
    if margin_state in ["WATCH", "PRESSURE", "CRITICAL"]:
        return "Margin Recovery"
    if mcr_state in ["COST_PRESSURE", "CRITICAL"]:
        return "People-Cost Control"
    if board_status == "Growth Ready":
        return "Scale-Up Selectively"
    if board_status == "Optimize Growth":
        return "Sustainable Growth"
    if board_status == "Selective Hiring":
        return "Selective Hiring"
    if board_status == "Capacity Alert":
        return "Capacity Review"
    if board_status == "Defensive Mode":
        return "Profit & Cash Protection"
    return "Balanced Optimization"


def classify_metric(label: str, value) -> Dict[str, str]:
    if pd.isna(value):
        return {"status": "UNKNOWN", "text": f"{label} belum tersedia."}
    v = float(value)
    if label == "Revenue Growth":
        if v >= 0.20:
            return {"status": "EXCELLENT", "text": "Revenue growth melampaui target 20%."}
        if v >= 0.10:
            return {"status": "GOOD", "text": "Revenue masih bertumbuh, namun belum mencapai target kuat."}
        if v >= 0:
            return {"status": "WATCH", "text": "Revenue tumbuh tipis dan perlu akselerasi pipeline."}
        return {"status": "PRESSURE", "text": "Revenue menurun; fokus pada recovery revenue."}
    if label == "RPE Growth":
        if v >= 0.15:
            return {"status": "STRONG", "text": "Produktivitas SDM meningkat kuat."}
        if v >= 0:
            return {"status": "STABLE", "text": "Produktivitas masih positif, tetapi belum optimal."}
        return {"status": "DECLINING", "text": "Produktivitas per karyawan menurun."}
    if label == "Headcount Growth":
        if v <= 0.10:
            return {"status": "CONTROLLED", "text": "Pertumbuhan HC masih terkendali."}
        if v <= 0.20:
            return {"status": "WATCH", "text": "HC tumbuh cukup cepat; validasi kebutuhan kapasitas."}
        return {"status": "AGGRESSIVE", "text": "HC tumbuh agresif dan berpotensi menekan MCR."}
    return {"status": "OK", "text": f"{label} terbaca."}


def build_executive_strengths(latest: pd.Series, bp: Dict, mq: Dict, mh: Dict, cr: Dict, board_score: Dict) -> List[str]:
    strengths = []
    if pd.notna(latest.get("Revenue_YoY")) and latest.get("Revenue_YoY") >= 0.20:
        strengths.append("Revenue growth melampaui target 20%.")
    if pd.notna(latest.get("RPE_YoY")) and latest.get("RPE_YoY") >= 0.15:
        strengths.append("RPE growth kuat; produktivitas SDM meningkat.")
    if pd.notna(latest.get("Headcount_YoY")) and latest.get("Headcount_YoY") <= 0.10:
        strengths.append("Headcount growth terkendali sehingga organisasi tetap efisien.")
    if mh.get("state") in ["HEALTHY", "WATCH"]:
        strengths.append(f"MCR berada pada level {mh.get('state')} dan masih dapat dikelola.")
    if mq.get("state") in ["STRONG", "HEALTHY"]:
        strengths.append(f"Margin Quality {mq.get('state')} mendukung profitabilitas.")
    if cr.get("state") == "LOW":
        strengths.append("Capacity Risk LOW; organisasi masih memiliki ruang kapasitas.")
    if board_score.get("score", 0) >= 70:
        strengths.append("Board Score menunjukkan arah perusahaan masih positif.")
    return strengths[:6] if strengths else ["Belum ada kekuatan dominan; perlu validasi data dan tren KPI."]


def build_executive_concerns(latest: pd.Series, bp: Dict, mq: Dict, mh: Dict, cr: Dict) -> List[str]:
    concerns = []
    if pd.notna(latest.get("Revenue_YoY")) and latest.get("Revenue_YoY") < 0:
        concerns.append("Revenue menurun dibanding periode sebelumnya.")
    if pd.notna(latest.get("RPE_YoY")) and latest.get("RPE_YoY") < 0:
        concerns.append("RPE menurun; produktivitas per karyawan melemah.")
    if pd.notna(latest.get("Headcount_YoY")) and latest.get("Headcount_YoY") > 0.15:
        concerns.append("Headcount tumbuh cepat dan perlu dikaitkan dengan revenue/pipeline.")
    if mh.get("state") in ["COST_PRESSURE", "CRITICAL"]:
        concerns.append(f"MCR berada pada status {mh.get('state')} dan dapat menekan profitabilitas.")
    if mh.get("state") == "ULTRA_EFFICIENCY":
        concerns.append("MCR sangat rendah; validasi risiko under-capacity atau overload tim.")
    if mq.get("state") in ["WATCH", "PRESSURE", "CRITICAL"]:
        concerns.append(f"Margin Quality {mq.get('state')}; perlu review pricing, COGS, dan delivery cost.")
    if cr.get("state") in ["HIGH", "CRITICAL"]:
        concerns.append(f"Capacity Risk {cr.get('state')}; bottleneck operasional perlu segera ditangani.")
    d = cr.get("details", {})
    if d.get("total_overtime", 0) > 250:
        concerns.append("Overtime tinggi; indikasi beban kerja tidak seimbang.")
    if d.get("total_backlog", 0) >= 10:
        concerns.append("Backlog meningkat; risiko keterlambatan delivery.")
    if d.get("total_sla_breach", 0) > 0:
        concerns.append("Terdapat SLA breach yang perlu ditindaklanjuti.")
    return concerns[:7] if concerns else ["Belum ada concern kritikal; tetap lakukan monitoring rutin."]


def build_role_recommendations(board_status: str, mq_state: str, mh_state: str, cap_state: str) -> Dict[str, List[str]]:
    ceo, cfo, coo, chro, sales = [], [], [], [], []
    if board_status in ["Growth Ready", "Optimize Growth"]:
        ceo += ["Lanjutkan growth strategy secara selektif.", "Fokus ekspansi pada bisnis/proyek dengan margin sehat."]
        sales += ["Perkuat pipeline high-margin project.", "Prioritaskan account dengan recurring revenue dan delivery feasibility tinggi."]
    elif board_status in ["Selective Hiring", "Capacity Alert"]:
        ceo += ["Tahan ekspansi non-critical sampai bottleneck utama jelas.", "Prioritaskan kapasitas delivery yang langsung menopang revenue."]
        sales += ["Hindari deal low-margin yang memperberat delivery.", "Sinkronkan target sales dengan kapasitas operasi."]
    else:
        ceo += ["Masuk mode proteksi margin dan cash.", "Tunda ekspansi besar sampai profitabilitas pulih."]
        sales += ["Fokus recovery revenue dan retention account utama.", "Review pipeline yang berisiko margin rendah."]
    if mq_state in ["WATCH", "PRESSURE", "CRITICAL"]:
        cfo += ["Lakukan margin quality review per project.", "Review pricing, COGS, scope creep, subcontractor, dan rework."]
    else:
        cfo += ["Pertahankan Gross Margin minimal pada zona healthy.", "Jaga disiplin pricing dan cost-to-serve."]
    if mh_state in ["COST_PRESSURE", "CRITICAL"]:
        cfo += ["Perketat approval manpower cost dan hiring.", "Tetapkan guardrail MCR dan review payroll impact."]
    else:
        cfo += ["Pertahankan MCR dalam zona sehat/watch yang disepakati.", "Gunakan MCR sebagai guardrail sebelum menambah HC."]
    if cap_state in ["HIGH", "CRITICAL"]:
        coo += ["Lakukan capacity review lintas fungsi dalam 30 hari.", "Turunkan backlog, overtime, dan SLA breach.", "Identifikasi bottleneck delivery dan dependency role kritikal."]
        chro += ["Lakukan selective hiring untuk fungsi bottleneck.", "Bangun retention dan succession plan untuk role kritikal.", "Perkuat upskilling/cross-skilling untuk mengurangi dependency."]
    else:
        coo += ["Pertahankan utilisasi dan SLA pada level terkendali.", "Perbaiki proses delivery sebelum menambah kapasitas."]
        chro += ["Tidak perlu mass hiring; fokus pada produktivitas dan capability building.", "Monitor turnover dan engagement role kritikal."]
    return {"CEO": ceo[:4], "CFO": cfo[:4], "COO": coo[:4], "CHRO": chro[:4], "Sales Director": sales[:4]}


def build_risk_register(latest: pd.Series, mq: Dict, mh: Dict, cr: Dict) -> pd.DataFrame:
    risks = []
    def add(risk, probability, impact, level, mitigation):
        risks.append({"Risk": risk, "Probability": probability, "Impact": impact, "Level": level, "Mitigation": mitigation})
    if cr.get("state") in ["HIGH", "CRITICAL"]:
        add("Capacity / Delivery Bottleneck", "High", "High", cr.get("state"), "Capacity review, selective hiring, workload redistribution, backlog burn-down.")
    else:
        add("Capacity / Delivery Bottleneck", "Medium", "Medium", cr.get("state"), "Monitor utilization, overtime, backlog, SLA, and dependency monthly.")
    if mq.get("state") in ["WATCH", "PRESSURE", "CRITICAL"]:
        add("Margin Compression", "High", "High", mq.get("state"), "Pricing review, COGS control, scope control, rework reduction.")
    else:
        add("Margin Compression", "Medium", "Medium", mq.get("state"), "Maintain pricing discipline and project profitability review.")
    if mh.get("state") in ["COST_PRESSURE", "CRITICAL"]:
        add("People Cost Pressure", "High", "Medium", mh.get("state"), "Hiring approval, payroll review, benefit/overtime control.")
    elif mh.get("state") == "ULTRA_EFFICIENCY":
        add("Under-Capacity / Overload", "Medium", "High", mh.get("state"), "Validate workload, overtime, backlog, and critical role dependency.")
    else:
        add("People Cost Pressure", "Low", "Medium", mh.get("state"), "Maintain MCR guardrail and review manpower ROI.")
    if pd.notna(latest.get("Revenue_YoY")) and latest.get("Revenue_YoY") < 0.10:
        add("Revenue Growth Slowdown", "Medium", "High", "WATCH", "Pipeline acceleration, account retention, and focus on high-probability deals.")
    else:
        add("Revenue Growth Slowdown", "Low", "High", "LOW", "Maintain sales pipeline quality and delivery feasibility.")
    return pd.DataFrame(risks)


def build_30_60_90_plan(board_status: str, mq_state: str, mh_state: str, cap_state: str) -> Dict[str, List[str]]:
    d30 = ["Validate KPI source data and close data gaps.", "Review top risks with CEO/CFO/COO/CHRO."]
    d60 = ["Run department-level productivity and capacity review.", "Align hiring and budget with revenue pipeline."]
    d90 = ["Execute approved actions and monitor KPI movement.", "Present updated Board Score and risk status."]
    if cap_state in ["HIGH", "CRITICAL"]:
        d30 += ["Review overtime, backlog, SLA breach, and role dependency.", "Identify bottleneck departments and critical roles."]
        d60 += ["Start selective hiring or redeploy resources to bottleneck functions.", "Implement backlog burn-down plan."]
        d90 += ["Measure reduction in overtime/backlog and reassess capacity score."]
    if mq_state in ["WATCH", "PRESSURE", "CRITICAL"]:
        d30 += ["Review project margin and COGS drivers."]
        d60 += ["Implement pricing and scope control mechanism."]
        d90 += ["Recheck margin recovery and project profitability."]
    if mh_state in ["COST_PRESSURE", "CRITICAL"]:
        d30 += ["Freeze non-critical hiring pending ROI review."]
        d60 += ["Review payroll, overtime, bonus, and benefit structure."]
        d90 += ["Set MCR guardrail and approval workflow."]
    if board_status in ["Growth Ready", "Optimize Growth"]:
        d60 += ["Prioritize high-margin growth initiatives."]
        d90 += ["Prepare selective scale-up roadmap."]
    return {"0-30 Days": d30[:5], "31-60 Days": d60[:5], "61-90 Days": d90[:5]}


def build_final_conclusion(year_label: str, analysis_mode: str, board_score: Dict, latest: pd.Series, mq: Dict, mh: Dict, cr: Dict) -> str:
    status = board_score.get("label", "Balanced")
    base = (
        f"Berdasarkan seluruh indikator Board pada {year_label} dalam mode {analysis_mode}, "
        f"perusahaan berada pada fase **{status}** dengan Board Score **{board_score.get('score')}/100**. "
        f"Revenue Growth **{_pct(latest.get('Revenue_YoY'))}**, RPE Growth **{_pct(latest.get('RPE_YoY'))}**, "
        f"Headcount Growth **{_pct(latest.get('Headcount_YoY'))}**, MCR **{_pct(latest.get('MCR_Pct'))}**, "
        f"dan Gross Margin **{_pct(latest.get('Gross_Margin_Pct'))}**. "
    )
    if status == "Growth Ready":
        base += "Perusahaan siap scale-up terukur karena growth dan produktivitas berada pada level kuat. "
    elif status == "Optimize Growth":
        base += "Arah perusahaan positif, namun perlu optimasi margin, kapasitas, atau struktur biaya agar pertumbuhan berkelanjutan. "
    elif status == "Selective Hiring":
        base += "Perusahaan masih dapat tumbuh, tetapi ekspansi HC harus selektif dan berbasis bottleneck yang terbukti. "
    elif status == "Capacity Alert":
        base += "Pertumbuhan mulai dibatasi risiko kapasitas sehingga prioritas utama adalah stabilisasi delivery dan workload. "
    else:
        base += "Perusahaan perlu masuk mode proteksi margin, cash, dan fungsi kritikal sebelum ekspansi baru. "
    if cr.get("state") in ["HIGH", "CRITICAL"]:
        base += "Capacity Risk menjadi perhatian utama karena ada indikasi bottleneck, overtime, backlog, SLA breach, atau dependency role kritikal. "
    if mq.get("state") in ["WATCH", "PRESSURE", "CRITICAL"]:
        base += "Margin Quality perlu dipulihkan melalui pricing review, COGS control, dan disiplin scope delivery. "
    if mh.get("state") in ["COST_PRESSURE", "CRITICAL"]:
        base += "MCR harus dikendalikan agar people-cost tidak menekan profitabilitas. "
    if mh.get("state") == "ULTRA_EFFICIENCY" and cr.get("state") in ["HIGH", "CRITICAL"]:
        base += "MCR terlalu efisien perlu divalidasi karena bisa menyembunyikan risiko under-capacity atau overload tim. "
    base += "Rekomendasi akhir: jalankan growth secara selektif, lindungi margin, gunakan MCR sebagai guardrail biaya, dan prioritaskan capacity review pada fungsi bottleneck."
    return base


def confidence_level(board_score: Dict, mq: Dict, mh: Dict, cr: Dict) -> Tuple[int, str]:
    score = 85
    if mq.get("state") in ["WATCH", "PRESSURE", "CRITICAL"]:
        score -= 8
    if mh.get("state") in ["ULTRA_EFFICIENCY", "COST_PRESSURE", "CRITICAL"]:
        score -= 8
    if cr.get("state") in ["HIGH", "CRITICAL"]:
        score -= 10
    if board_score.get("label") in ["Capacity Alert", "Defensive Mode"]:
        score -= 7
...
# =========================
# EXECUTIVE BOARD INTELLIGENCE REPORT
# =========================
st.markdown("## 9. Executive Board Intelligence Report")

current_board_score = board_score_engine(latest, targets, cr["state"])
year_label = str(period_info.get("Period_Label", year_sel))
priority_focus = board_priority_focus(current_board_score["label"], cr["state"], mq["state"], mh["state"])
conf_score, conf_label = confidence_level(current_board_score, mq, mh, cr)

st.caption(
    f"Dynamic annual assessment for {year_label} | Mode: {analysis_mode} | "
    f"Conclusion Confidence: {conf_score}/100 ({conf_label})"
)

er1, er2, er3, er4 = st.columns(4)
with er1:
    st.metric("Overall Board Score", f"{current_board_score['score']}/100", current_board_score["label"])
with er2:
    st.metric("Priority Focus", priority_focus)
with er3:
    st.metric("Capacity Risk", cr["state"], f"{cr['score']}/100")
with er4:
    st.metric("Confidence Level", f"{conf_score}/100", conf_label)

st.markdown("#### Executive Narrative")
st.info(build_final_conclusion(year_label, analysis_mode, current_board_score, latest, mq, mh, cr))

st.markdown("#### Executive Findings")
finding_cols = st.columns(6)
rev_assess = classify_metric("Revenue Growth", latest.get("Revenue_YoY"))
rpe_assess = classify_metric("RPE Growth", latest.get("RPE_YoY"))
hc_assess = classify_metric("Headcount Growth", latest.get("Headcount_YoY"))

with finding_cols[0]:
    st.metric("Revenue", rev_assess["status"], _pct(latest.get("Revenue_YoY")))
    st.caption(rev_assess["text"])
with finding_cols[1]:
    st.metric("Productivity", rpe_assess["status"], _pct(latest.get("RPE_YoY")))
    st.caption(rpe_assess["text"])
with finding_cols[2]:
    st.metric("Headcount", hc_assess["status"], _pct(latest.get("Headcount_YoY")))
    st.caption(hc_assess["text"])
with finding_cols[3]:
    st.metric("MCR", mh["state"], _pct(latest.get("MCR_Pct")))
    st.caption(mh["hint"])
with finding_cols[4]:
    st.metric("Margin", mq["state"], _pct(latest.get("Gross_Margin_Pct")))
    st.caption(mq["hint"])
with finding_cols[5]:
    st.metric("Capacity", cr["state"], f"{cr['score']}/100")
    st.caption(cr.get("recommendation", "-"))

sc1, sc2 = st.columns(2)
with sc1:
    st.markdown("#### Top Strength")
    for s in build_executive_strengths(latest, bp, mq, mh, cr, current_board_score):
        st.success("✓ " + s)
with sc2:
    st.markdown("#### Top Concern")
    for c in build_executive_concerns(latest, bp, mq, mh, cr):
        st.warning("⚠ " + c)

st.markdown("#### Board Recommendation by Function")
role_recs = build_role_recommendations(current_board_score["label"], mq["state"], mh["state"], cr["state"])
role_cols = st.columns(5)
for col, (role, recs) in zip(role_cols, role_recs.items()):
    with col:
        st.markdown(f"**{role}**")
        for r in recs:
            st.write("• " + r)

rr1, rr2 = st.columns([1.1, 1.0])
with rr1:
    st.markdown("#### Risk Register")
    st.dataframe(build_risk_register(latest, mq, mh, cr), use_container_width=True, hide_index=True)

with rr2:
    st.markdown("#### 30-60-90 Days Action Plan")
    plan = build_30_60_90_plan(current_board_score["label"], mq["state"], mh["state"], cr["state"])
    for phase, actions in plan.items():
        with st.expander(phase, expanded=(phase == "0-30 Days")):
            for a in actions:
                st.write("• " + a)

st.markdown("#### Board Evolution by Year")
evo_rows = []
for _, r in yr_range.iterrows():
    _cr = capacity_risk(r, data["capacity"], targets)
    _bs = board_score_engine(r, targets, _cr["state"])
    _mq = margin_quality(r.get("Gross_Margin_Pct"), targets)
    _mh = mcr_health(r.get("MCR_Pct"), targets)
    _focus = board_priority_focus(_bs["label"], _cr["state"], _mq["state"], _mh["state"])
    _period = get_calendar_info(data.get("calendar", pd.DataFrame()), int(r.get("Year")))
    evo_rows.append({
        "Year": int(r.get("Year")),
        "Period": _period.get("Period_Label", int(r.get("Year"))),
        "Board Score": _bs["score"],
        "Board Status": f"{_status_icon(_bs['label'])} {_bs['label']}",
        "Revenue Growth": _pct(r.get("Revenue_YoY")),
        "RPE Growth": _pct(r.get("RPE_YoY")),
        "MCR": _pct(r.get("MCR_Pct")),
        "Margin": _pct(r.get("Gross_Margin_Pct")),
        "Capacity": _cr["state"],
        "Focus": _focus,
    })
...
if not evo_df.empty:
    fig = px.line(evo_df, x="Year", y="Board Score", markers=True, text="Board Score", color="Board Status", title="Board Evolution Score")
    fig.update_traces(textposition="top center")
    fig.update_yaxes(range=[0, 100])
    fig.update_layout(margin=dict(l=10, r=10, t=45, b=10))
    st.plotly_chart(fig, use_container_width=True)

st.markdown("#### Board Intelligence Insight")
insights = []
if pd.notna(latest.get("Revenue_YoY")) and pd.notna(latest.get("Headcount_YoY")):
    if latest.get("Revenue_YoY") > latest.get("Headcount_YoY"):
        insights.append("Revenue tumbuh lebih cepat daripada headcount; indikasi pertumbuhan didorong oleh produktivitas, bukan hanya penambahan manpower.")
    else:
        insights.append("Headcount tumbuh lebih cepat daripada revenue; validasi apakah tambahan kapasitas sudah menghasilkan revenue yang sepadan.")
if mq["state"] in ["WATCH", "PRESSURE", "CRITICAL"]:
    insights.append("Revenue perlu dibaca bersama margin karena pertumbuhan revenue belum tentu berkualitas jika Gross Margin melemah.")
if mh["state"] == "ULTRA_EFFICIENCY" and cr["state"] in ["HIGH", "CRITICAL"]:
    insights.append("MCR sangat efisien tetapi Capacity Risk tinggi; validasi risiko under-capacity, overload, atau bottleneck delivery.")
if cr["state"] in ["HIGH", "CRITICAL"]:
    insights.append("Capacity Risk tinggi dapat membatasi pertumbuhan berikutnya meskipun revenue dan RPE terlihat baik.")
if not insights:
    insights.append("Tidak ada konflik besar antar KPI; fokus utama adalah mempertahankan growth, margin, MCR, dan kapasitas secara konsisten.")

for i in insights:
    st.info(i)

with st.expander("How to Read Executive Board Intelligence Report"):
    st.markdown("""
    - **Overall Board Score** merangkum Revenue Growth, RPE Growth, Margin Quality, MCR Health, dan Capacity Risk.
    - **Priority Focus** menunjukkan tema keputusan utama tahun terpilih.
    - **Executive Narrative** adalah kesimpulan otomatis berdasarkan kondisi KPI tahun yang dipilih.
    - **Top Strength** menunjukkan faktor yang mendukung pertumbuhan.
    - **Top Concern** menunjukkan risiko yang harus dimitigasi.
    - **Board Recommendation by Function** membagi action untuk CEO, CFO, COO, CHRO, dan Sales Director.
    - **Risk Register** menjelaskan risiko, level, dan mitigasi.
    - **30-60-90 Days Action Plan** memberikan langkah eksekusi jangka pendek.
    - **Board Evolution** membandingkan kondisi antar tahun sehingga penilaian tidak statis.
    - **Board Intelligence Insight** menjelaskan konflik atau sinyal penting antar KPI.
    """)
