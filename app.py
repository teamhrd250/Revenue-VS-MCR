...
# app.py
# MCR vs Revenue Platform V3 — Board & CFO Edition
# Streamlit dashboard based on Excel data model:
# MCR_vs_Revenue_Platform_V3_Board_CFO_Template.xlsx
...
# =========================
st.set_page_config(
    page_title="MCR vs Revenue Platform V3",
    page_icon="📊",
    layout="wide",
)
    ...
    with col_title:
        st.markdown(
            ...
            <div style="padding:4px 0 10px 0;">
                <div style="font-size:36px; font-weight:800; line-height:1.1;">MCR vs Revenue Platform V3</div>
                <div style="font-size:15px; color:#64748b; margin-top:6px;">Board & CFO Edition - income statement, productivity, margin quality, MCR and capacity risk.</div>
            </div>
            """,
...
def _first_existing(df: pd.DataFrame, names: List[str], default: str = "") -> str:
    for n in names:
        if n in df.columns:
            return n
    return default
...
def _financial_col(df: pd.DataFrame, names: List[str]) -> pd.Series:
...
def clean_and_calc(sheets: Dict[str, pd.DataFrame], analysis_mode: str = "Actual") -> Dict[str, pd.DataFrame]:
    ...
    # - Annualized mode converts every amount to full-year equivalent: amount * (12 / Months_Closed).
    # - YTD Comparable mode converts every year to the same month cut-off as the latest YTD year.
    #   Example: if 2026 has Months_Closed = 4, then 2026 stays actual 4M and 2025 FY becomes 4/12 of FY.
    #   This is an apple-to-apple proxy when monthly 2025 actual is not available.
    _calendar_tmp = calendar.copy()
    _calendar_tmp["Months_Closed"] = pd.to_numeric(_calendar_tmp["Months_Closed"], errors="coerce").fillna(12).clip(lower=1, upper=12)
    _ytd_months = _calendar_tmp.loc[_calendar_tmp["Months_Closed"] < 12, "Months_Closed"]
    comparable_months = int(_ytd_months.max()) if len(_ytd_months) else 12
    comparable_months_map = {int(r["Year"]): comparable_months for _, r in _calendar_tmp.iterrows()}
    ytd_comparable_factor_map = {
        int(r["Year"]): float(comparable_months / float(r["Months_Closed"]))
        for _, r in _calendar_tmp.iterrows()
    }
    ...
    if analysis_mode in ["Annualized", "YTD Comparable"]:
        _rev_factor_map = annualization_map if analysis_mode == "Annualized" else ytd_comparable_factor_map
        _rev_factor = rev["Year"].astype("float").map(_rev_factor_map).fillna(1.0)
    ...
    if analysis_mode in ["Annualized", "YTD Comparable"]:
        _pay_factor_map = annualization_map if analysis_mode == "Annualized" else ytd_comparable_factor_map
        _pay_factor = pay["Year"].astype("float").map(_pay_factor_map).fillna(1.0)
        for _c in ["Payroll_Gross", "Overtime", "Bonus", "Benefits", "Employer_Tax", "Total_Manpower_Cost"]:
            if _c in pay.columns:
    ...
    if "REVENUE_BY_DEPT" in sheets and not sheets["REVENUE_BY_DEPT"].empty:
        ...
        if analysis_mode in ["Annualized", "YTD Comparable"]:
            _rd_factor_map = annualization_map if analysis_mode == "Annualized" else ytd_comparable_factor_map
            _rd_factor = rd["Year"].astype("float").map(_rd_factor_map).fillna(1.0)
    ...
    if "PROJECT_MARGIN" in sheets and not sheets["PROJECT_MARGIN"].empty:
        ...
        if analysis_mode in ["Annualized", "YTD Comparable"]:
            _pm_factor_map = annualization_map if analysis_mode == "Annualized" else ytd_comparable_factor_map
            _pm_factor = pm["Year"].astype("float").map(_pm_factor_map).fillna(1.0)
    ...
    # Financials / Income Statement Intelligence
    # If FINANCIALS exists, it follows the CFO P&L structure:
    # Revenue -> Net Revenue -> Gross Profit -> EBIT -> PBT -> Net Profit.
    if "FINANCIALS" in sheets and not sheets["FINANCIALS"].empty:
        fin = sheets["FINANCIALS"].copy()
        fin.columns = [str(c).strip() for c in fin.columns]
        fin["Year"] = pd.to_numeric(fin["Year"], errors="coerce").astype("Int64")
        fin = fin.dropna(subset=["Year"]).copy()

        # Flexible aliases support both old V3 and CFO statement labels.
        fin["Project_Revenue"] = _financial_col(fin, ["Project_Revenue", "Revenue", "Pendapatan", "Penerimaan_Proyek"])
        fin["Sales_Discount"] = _financial_col(fin, ["Sales_Discount", "Discount_Penjualan", "Diskon_Penjualan"])
        fin["Net_Revenue"] = _financial_col(fin, ["Net_Revenue"])
        missing_net = fin["Net_Revenue"].eq(0)
        fin.loc[missing_net, "Net_Revenue"] = fin.loc[missing_net, "Project_Revenue"] - fin.loc[missing_net, "Sales_Discount"]
        fin["COGS_Direct"] = _financial_col(fin, ["COGS_Direct", "COGS", "Harga_Pokok_Penjualan", "HPP"])
        fin["Project_Overhead"] = _financial_col(fin, ["Project_Overhead", "Overhead"])
        fin["Biaya_Administrasi"] = _financial_col(fin, ["Biaya_Administrasi", "Admin_OPEX", "Admin_OPEX_Non_Manpower", "Administration_Expense"])
        fin["Biaya_Marketing"] = _financial_col(fin, ["Biaya_Marketing", "Selling_OPEX", "Selling_OPEX_Non_Manpower", "Marketing_Expense"])
        fin["Biaya_Litbang_RnD"] = _financial_col(fin, ["Biaya_Litbang_RnD", "Biaya_Litbang", "RND_Expense", "R&D_Expense"])
        fin["Other_Income"] = _financial_col(fin, ["Other_Income", "Pendapatan_Lain_Lain"])
        fin["Other_Expense"] = _financial_col(fin, ["Other_Expense", "Biaya_Lain_Lain"])
        fin["Interest_Expense"] = _financial_col(fin, ["Interest_Expense", "Beban_Bunga_Pinjaman", "Biaya_Bunga_Pinjaman"])
        fin["PPh_29"] = _financial_col(fin, ["PPh_29", "PPH_Ps_29"])
        fin["PPh_23_42"] = _financial_col(fin, ["PPh_23_42", "PPH_Ps_4_2", "PPh_4_2"])
        fin["Total_Tax"] = _financial_col(fin, ["Total_Tax", "Tax_Expense"])
        missing_tax = fin["Total_Tax"].eq(0)
        fin.loc[missing_tax, "Total_Tax"] = fin.loc[missing_tax, "PPh_29"] + fin.loc[missing_tax, "PPh_23_42"]

        financial_amount_cols = [
            "Project_Revenue", "Sales_Discount", "Net_Revenue", "COGS_Direct", "Project_Overhead",
            "Biaya_Administrasi", "Biaya_Marketing", "Biaya_Litbang_RnD", "Other_Income", "Other_Expense",
            "Interest_Expense", "PPh_29", "PPh_23_42", "Total_Tax"
        ]
        if analysis_mode in ["Annualized", "YTD Comparable"]:
            _fin_factor_map = annualization_map if analysis_mode == "Annualized" else ytd_comparable_factor_map
            _fin_factor = fin["Year"].astype("float").map(_fin_factor_map).fillna(1.0)
            for _c in financial_amount_cols:
                fin[_c] = fin[_c] * _fin_factor

        fin["Gross_Profit_Financial"] = fin["Net_Revenue"] - fin["COGS_Direct"] - fin["Project_Overhead"]
        fin["Total_Biaya_Operasional"] = fin["Biaya_Administrasi"] + fin["Biaya_Marketing"] + fin["Biaya_Litbang_RnD"]
        fin["EBIT"] = fin["Gross_Profit_Financial"] - fin["Total_Biaya_Operasional"]
        fin["Total_Other_Income_Expense"] = fin["Other_Income"] - fin["Other_Expense"] - fin["Interest_Expense"]
        fin["PBT"] = fin["EBIT"] + fin["Total_Other_Income_Expense"]
        fin["Net_Profit"] = fin["PBT"] - fin["Total_Tax"]
        fin["Gross_Margin_Financial_Pct"] = np.where(fin["Net_Revenue"] > 0, fin["Gross_Profit_Financial"] / fin["Net_Revenue"], np.nan)
        fin["Operating_Margin_Pct"] = np.where(fin["Net_Revenue"] > 0, fin["EBIT"] / fin["Net_Revenue"], np.nan)
        fin["Net_Margin_Pct"] = np.where(fin["Net_Revenue"] > 0, fin["Net_Profit"] / fin["Net_Revenue"], np.nan)
        fin["Admin_OPEX_Ratio"] = np.where(fin["Net_Revenue"] > 0, fin["Biaya_Administrasi"] / fin["Net_Revenue"], np.nan)
        fin["OPEX_Ratio"] = np.where(fin["Net_Revenue"] > 0, fin["Total_Biaya_Operasional"] / fin["Net_Revenue"], np.nan)
        fin["Interest_Ratio"] = np.where(fin["Net_Revenue"] > 0, fin["Interest_Expense"] / fin["Net_Revenue"], np.nan)
        fin["Tax_Ratio"] = np.where(fin["Net_Revenue"] > 0, fin["Total_Tax"] / fin["Net_Revenue"], np.nan)
        fin["Profit_Conversion_Pct"] = np.where(fin["Gross_Profit_Financial"] > 0, fin["Net_Profit"] / fin["Gross_Profit_Financial"], np.nan)
        fin_tot = fin[[
            "Year", "Project_Revenue", "Sales_Discount", "Net_Revenue", "COGS_Direct", "Project_Overhead",
            "Gross_Profit_Financial", "Biaya_Administrasi", "Biaya_Marketing", "Biaya_Litbang_RnD",
            "Total_Biaya_Operasional", "EBIT", "Other_Income", "Other_Expense", "Interest_Expense",
            "Total_Other_Income_Expense", "PBT", "PPh_29", "PPh_23_42", "Total_Tax", "Net_Profit",
            "Gross_Margin_Financial_Pct", "Operating_Margin_Pct", "Net_Margin_Pct", "Admin_OPEX_Ratio",
            "OPEX_Ratio", "Interest_Ratio", "Tax_Ratio", "Profit_Conversion_Pct"
        ]].copy()
    else:
        fin_tot = yr[["Year", "Total_Revenue", "Total_COGS", "Gross_Profit", "Gross_Margin_Pct"]].copy()
        fin_tot["Project_Revenue"] = fin_tot["Total_Revenue"]
        fin_tot["Sales_Discount"] = 0.0
        fin_tot["Net_Revenue"] = fin_tot["Total_Revenue"]
        fin_tot["COGS_Direct"] = fin_tot["Total_COGS"]
        fin_tot["Project_Overhead"] = 0.0
        fin_tot["Gross_Profit_Financial"] = fin_tot["Gross_Profit"]
        fin_tot["Biaya_Administrasi"] = 0.0
        fin_tot["Biaya_Marketing"] = 0.0
        fin_tot["Biaya_Litbang_RnD"] = 0.0
        fin_tot["Total_Biaya_Operasional"] = 0.0
        fin_tot["EBIT"] = fin_tot["Gross_Profit_Financial"]
        fin_tot["Other_Income"] = 0.0
        fin_tot["Other_Expense"] = 0.0
        fin_tot["Interest_Expense"] = 0.0
        fin_tot["Total_Other_Income_Expense"] = 0.0
        fin_tot["PBT"] = fin_tot["EBIT"]
        fin_tot["PPh_29"] = 0.0
        fin_tot["PPh_23_42"] = 0.0
        fin_tot["Total_Tax"] = 0.0
        fin_tot["Net_Profit"] = fin_tot["PBT"]
        fin_tot["Gross_Margin_Financial_Pct"] = fin_tot["Gross_Margin_Pct"]
        fin_tot["Operating_Margin_Pct"] = np.where(fin_tot["Net_Revenue"] > 0, fin_tot["EBIT"] / fin_tot["Net_Revenue"], np.nan)
        fin_tot["Net_Margin_Pct"] = np.where(fin_tot["Net_Revenue"] > 0, fin_tot["Net_Profit"] / fin_tot["Net_Revenue"], np.nan)
        fin_tot["Admin_OPEX_Ratio"] = 0.0
        fin_tot["OPEX_Ratio"] = 0.0
        fin_tot["Interest_Ratio"] = 0.0
        fin_tot["Tax_Ratio"] = 0.0
        fin_tot["Profit_Conversion_Pct"] = 1.0

    yr = yr.merge(fin_tot, on="Year", how="left")
    # Use financial gross margin as CFO view when FINANCIALS exists.
    yr["Gross_Margin_Pct"] = yr["Gross_Margin_Financial_Pct"].combine_first(yr["Gross_Margin_Pct"])
...
def operating_margin_quality(op_margin, targets: Dict[str, float]) -> Dict[str, str]:
    strong = get_target(targets, "Operating_Margin_Strong_Min", 0.15)
    healthy = get_target(targets, "Operating_Margin_Healthy_Min", 0.10)
    watch = get_target(targets, "Operating_Margin_Watch_Min", 0.05)
    pressure = get_target(targets, "Operating_Margin_Pressure_Min", 0.02)
    if pd.isna(op_margin):
        return {"state": "UNKNOWN", "tone": "info", "hint": "Operating Margin belum tersedia."}
    if op_margin >= strong:
        return {"state": "STRONG", "tone": "success", "hint": "Laba operasi sangat kuat setelah biaya operasional."}
    if op_margin >= healthy:
        return {"state": "HEALTHY", "tone": "success", "hint": "Laba operasi sehat; biaya operasional masih terkendali."}
    if op_margin >= watch:
        return {"state": "WATCH", "tone": "warning", "hint": "Operating Margin perlu dipantau; cek biaya administrasi, marketing, dan litbang."}
    if op_margin >= pressure:
        return {"state": "PRESSURE", "tone": "warning", "hint": "Operating profit mulai tertekan oleh biaya operasional."}
    return {"state": "CRITICAL", "tone": "error", "hint": "Operating Margin kritikal; perlu review struktur biaya."}


def net_margin_quality(net_margin, targets: Dict[str, float]) -> Dict[str, str]:
    strong = get_target(targets, "Net_Margin_Strong_Min", 0.06)
    healthy = get_target(targets, "Net_Margin_Healthy_Min", 0.04)
    watch = get_target(targets, "Net_Margin_Watch_Min", 0.02)
    pressure = get_target(targets, "Net_Margin_Pressure_Min", 0.01)
    if pd.isna(net_margin):
        return {"state": "UNKNOWN", "tone": "info", "hint": "Net Margin belum tersedia."}
    if net_margin >= strong:
        return {"state": "STRONG", "tone": "success", "hint": "Laba bersih kuat setelah bunga dan pajak."}
    if net_margin >= healthy:
        return {"state": "HEALTHY", "tone": "success", "hint": "Laba bersih sehat untuk bisnis project-based."}
    if net_margin >= watch:
        return {"state": "WATCH", "tone": "warning", "hint": "Net Margin tipis; cek bunga, pajak, dan operating expense."}
    if net_margin >= pressure:
        return {"state": "PRESSURE", "tone": "warning", "hint": "Net Margin mulai tertekan; ruang laba bersih sangat terbatas."}
    return {"state": "CRITICAL", "tone": "error", "hint": "Net Margin kritikal; pertumbuhan revenue belum menjadi laba bersih."}


def ratio_guardrail(value, max_value, label: str) -> Dict[str, str]:
    if pd.isna(value):
        return {"state": "UNKNOWN", "tone": "info", "hint": f"{label} belum tersedia."}
    if value <= max_value:
        return {"state": "CONTROLLED", "tone": "success", "hint": f"{label} masih dalam guardrail."}
    if value <= max_value * 1.25:
        return {"state": "WATCH", "tone": "warning", "hint": f"{label} sedikit di atas target; perlu monitoring."}
    return {"state": "PRESSURE", "tone": "error", "hint": f"{label} menekan profitabilitas."}
...
def board_score_engine(row: pd.Series, targets: Dict[str, float], cap_state: str = "LOW") -> Dict[str, object]:
    """Board Score Engine v4 - Board & CFO Edition.
    ...
    npm = row.get("Net_Margin_Pct")
    profit_conversion = row.get("Profit_Conversion_Pct")

    mq_state = margin_quality(gm, targets)["state"]
    mh_state = mcr_health(mcr, targets)["state"]
    op_state = operating_margin_quality(opm, targets)["state"]
    net_state = net_margin_quality(npm, targets)["state"]

    pc_min = get_target(targets, "Profit_Conversion_Healthy_Min", 0.15)
    if pd.isna(profit_conversion):
        pc_score = 50
    elif profit_conversion >= pc_min * 2:
        pc_score = 100
    elif profit_conversion >= pc_min:
        pc_score = 80
    elif profit_conversion >= pc_min * 0.5:
        pc_score = 50
    else:
        pc_score = 25
    ...
    components = {
        ...
        "Gross Margin": _score_from_margin_state(mq_state),
        "Operating Margin": _score_from_margin_state(op_state),
        "Net Margin": _score_from_margin_state(net_state),
    ...
    return {
        ...
        "mcr_state": mh_state,
        "operating_margin_state": op_state,
        "net_margin_state": net_state,
...
with st.sidebar:
    ...
    analysis_mode = st.radio(
        "Compare mode",
        ["Actual", "Annualized", "YTD Comparable"],
        index=2 if ((_calendar_preview["Months_Closed"] < 12).any() if not _calendar_preview.empty else False) else 0,
        help="Actual = angka sesuai data. Annualized = YTD disetahunkan. YTD Comparable = semua tahun dipotong ke jumlah bulan YTD yang sama untuk apple-to-apple."
    )
...
opq = operating_margin_quality(latest.get("Operating_Margin_Pct"), targets)
netq = net_margin_quality(latest.get("Net_Margin_Pct"), targets)
admin_guard = ratio_guardrail(latest.get("Admin_OPEX_Ratio"), get_target(targets, "Admin_OPEX_Ratio_Max", 0.15), "Admin OPEX Ratio")
interest_guard = ratio_guardrail(latest.get("Interest_Ratio"), get_target(targets, "Interest_Ratio_Max", 0.05), "Interest Ratio")
...
if analysis_mode == "Actual" and period_info.get("Months_Closed", 12) < 12:
    ...
    st.info("Annualized mode aktif: Revenue, COGS, Dept Revenue, Project Revenue/COGS, Financials, dan Manpower Cost disetahunkan. Headcount tetap sebagai average capacity. Jika Months_Closed = 4, maka faktor annualized = 3x; jika Months_Closed = 6, faktor = 2x.")
elif analysis_mode == "YTD Comparable":
    st.info(f"YTD Comparable mode aktif: semua tahun dikonversi ke basis {data.get('comparable_months', period_info.get('Months_Closed'))} bulan agar FY sebelumnya bisa dibandingkan apple-to-apple dengan YTD berjalan. Contoh: FY 2025 x {data.get('comparable_months', 12)}/12, sedangkan YTD 2026 tetap pada angka YTD bila Months_Closed sama.")

b1, b2, b3 = st.columns(3)
with b1:
    st.markdown(badge_html("Business Posture", bp["state"], bp["tone"], bp["hint"]), unsafe_allow_html=True)
with b2:
    st.markdown(badge_html("Gross Margin", mq["state"], mq["tone"], mq["hint"]), unsafe_allow_html=True)
with b3:
    st.markdown(badge_html("Operating Margin", opq["state"], opq["tone"], opq["hint"]), unsafe_allow_html=True)

b4, b5, b6 = st.columns(3)
with b4:
    st.markdown(badge_html("Net Margin", netq["state"], netq["tone"], netq["hint"]), unsafe_allow_html=True)
with b5:
...
with k1:
    st.metric("💰 Revenue", _money(latest.get("Net_Revenue"), currency), _pct(latest.get("Revenue_YoY")))
with k2:
    st.metric("📈 Gross Profit", _money(latest.get("Gross_Profit_Financial"), currency), _pct(latest.get("Gross_Margin_Pct")))
with k3:
    st.metric("🏢 EBIT", _money(latest.get("EBIT"), currency), _pct(latest.get("Operating_Margin_Pct")))
with k4:
    st.metric("✅ Net Profit", _money(latest.get("Net_Profit"), currency), _pct(latest.get("Net_Margin_Pct")))
...
memo = (
    f"Pada {period_info.get('Period_Label')} dalam mode **{analysis_mode}**, Net Revenue terbaca **{_money(latest.get('Net_Revenue'), currency)}**, "
    f"Gross Profit **{_money(latest.get('Gross_Profit_Financial'), currency)}** ({_pct(latest.get('Gross_Margin_Pct'))}), "
    f"EBIT **{_money(latest.get('EBIT'), currency)}** ({_pct(latest.get('Operating_Margin_Pct'))}), dan "
    f"Net Profit **{_money(latest.get('Net_Profit'), currency)}** ({_pct(latest.get('Net_Margin_Pct'))}). "
    f"People productivity: headcount **{_num(latest.get('Total_Headcount'))}**, RPE **{_money(latest.get('RPE'), currency)}**, MCR **{_pct(latest.get('MCR_Pct'))}**. "
...
with st.expander("How to Read Executive Summary"):
    st.markdown("""
    ### Cara Membaca Executive Summary V3

    Executive Summary sekarang mengikuti alur **Income Statement + People Productivity**:

    **Revenue → Gross Profit → EBIT → PBT → Net Profit → MCR/RPE/Capacity**

    Artinya dashboard tidak hanya bertanya apakah revenue naik, tetapi juga apakah revenue tersebut benar-benar berubah menjadi laba.

    ### KPI & Target Board/CFO

    | KPI | Target V3 | Cara Baca |
    |---|---:|---|
    | Revenue Growth | **>=20%** | Pertumbuhan pendapatan dianggap kuat |
    | Gross Margin Strong | **>35%** | Margin kotor sangat kuat |
    | Gross Margin Healthy | **25-35%** | Margin kotor sehat |
    | Operating Margin Strong | **>=15%** | EBIT margin sangat kuat untuk bisnis project-based |
    | Operating Margin Healthy | **10-15%** | Laba operasi sehat setelah biaya operasional |
    | Operating Margin Watch | **5-10%** | Biaya operasional mulai perlu dipantau |
    | Net Margin Strong | **>=6%** | Laba bersih sangat kuat untuk bisnis project-based/SI |
    | Net Margin Healthy | **4-6%** | Laba bersih sehat |
    | Net Margin Watch | **2-4%** | Laba bersih tipis; perlu monitoring bunga, pajak, dan OPEX |
    | Net Margin Pressure | **1-2%** | Laba bersih sangat terbatas |
    | MCR Healthy | **5-7%** | Zona sehat manpower cost terhadap revenue |
    | MCR Watch | **7-9%** | Zona pantau people-cost |
    | Admin OPEX Ratio | **<=15%** | Biaya administrasi masih terkendali |
    | Interest Ratio | **<=5%** | Beban bunga masih dalam guardrail |
    | Profit Conversion | **>=15%** | Minimal 15% Gross Profit berubah menjadi Net Profit |

    ### Interpretasi Cepat

    - Jika **Gross Margin sehat tetapi Net Margin rendah**, masalahnya biasanya ada pada **biaya operasional, bunga pinjaman, pajak, atau biaya lain-lain**.
    - Jika **Revenue naik tetapi EBIT turun**, cek **Admin OPEX, Marketing, Litbang/R&D, overhead, dan efektivitas biaya operasional**.
    - Jika **Net Margin 2-4%**, statusnya bukan otomatis buruk untuk bisnis project-based, tetapi harus dibaca sebagai **Watch**.
    - Jika **MCR rendah tetapi Capacity Risk tinggi**, efisiensi manpower perlu divalidasi karena bisa menandakan under-capacity atau overload.
    - Sheet **FINANCIALS** mengikuti laporan laba/rugi. Sheet **PAYROLL_YR** tetap dipakai untuk MCR dan produktivitas SDM.
    """)

# =========================
# INCOME STATEMENT INTELLIGENCE
# =========================
st.markdown("## 2. Income Statement Intelligence")

pl1, pl2 = st.columns([1.15, 1.0])
with pl1:
    waterfall = pd.DataFrame({
        "Stage": ["Project Revenue", "Discount", "COGS", "Overhead", "Gross Profit", "OPEX", "Other/Interest", "Tax", "Net Profit"],
        "Amount": [
            latest.get("Project_Revenue"),
            -latest.get("Sales_Discount"),
            -latest.get("COGS_Direct"),
            -latest.get("Project_Overhead"),
            latest.get("Gross_Profit_Financial"),
            -latest.get("Total_Biaya_Operasional"),
            latest.get("Total_Other_Income_Expense"),
            -latest.get("Total_Tax"),
    ...
    fig.update_layout(title=f"Income Statement Bridge — {year_sel}", height=420, margin=dict(l=10, r=10, t=45, b=10))
    st.plotly_chart(fig, use_container_width=True)

with pl2:
    st.markdown("#### CFO Reading")
    st.metric("Gross Margin", _pct(latest.get("Gross_Margin_Pct")), mq["state"])
    st.metric("Operating Margin", _pct(latest.get("Operating_Margin_Pct")), opq["state"])
    st.metric("Net Margin", _pct(latest.get("Net_Margin_Pct")), netq["state"])
    st.metric("Profit Conversion", _pct(latest.get("Profit_Conversion_Pct")))

    if netq["state"] in ["WATCH", "PRESSURE", "CRITICAL"]:
        st.warning("Net Profit perlu perhatian. Cek OPEX, bunga pinjaman, pajak, dan biaya lain-lain.")
    elif opq["state"] in ["WATCH", "PRESSURE", "CRITICAL"]:
        st.warning("Gross Profit masih perlu diuji terhadap biaya operasional. Fokus pada efisiensi OPEX.")
    else:
        st.success("Profitability masih berada dalam zona yang dapat dikelola.")

ic1, ic2 = st.columns(2)
with ic1:
    margin_trend = yr_range[["Year", "Gross_Margin_Pct", "Operating_Margin_Pct", "Net_Margin_Pct"]].melt(
        id_vars="Year", var_name="Metric", value_name="Margin"
    )
    fig = px.line(margin_trend, x="Year", y="Margin", color="Metric", markers=True, title="Margin Stack Trend")
    fig.update_yaxes(tickformat=".0%")
    fig.update_layout(height=360, margin=dict(l=10, r=10, t=45, b=10))
    st.plotly_chart(fig, use_container_width=True)
with ic2:
    ratio_rows = pd.DataFrame({
        "Cost Ratio": ["Admin OPEX", "Total OPEX", "Interest", "Tax"],
        "Value": [latest.get("Admin_OPEX_Ratio"), latest.get("OPEX_Ratio"), latest.get("Interest_Ratio"), latest.get("Tax_Ratio")],
    })
    fig = px.bar(ratio_rows, x="Cost Ratio", y="Value", text=ratio_rows["Value"].map(_pct), title="Cost Ratio vs Net Revenue")
    fig.update_yaxes(tickformat=".0%")
    fig.update_layout(height=360, margin=dict(l=10, r=10, t=45, b=10))
    st.plotly_chart(fig, use_container_width=True)

with st.expander("How to Read Income Statement Intelligence"):
    st.markdown("""
    - **Project Revenue** = penerimaan proyek/pendapatan sebelum diskon.
    - **Sales Discount** = pengurang revenue yang berasal dari strategi penjualan, bukan COGS.
    - **Net Revenue** = Project Revenue - Discount.
    - **Gross Profit** = Net Revenue - COGS - Project Overhead.
    - **EBIT / Operating Profit** = Gross Profit - biaya operasional administrasi/marketing/litbang.
    - **PBT** = EBIT + pendapatan/beban lain-lain - bunga pinjaman.
    - **Net Profit** = PBT - pajak.
    - **Profit Conversion** = Net Profit / Gross Profit. Semakin tinggi berarti laba kotor lebih berhasil berubah menjadi laba bersih.

    Untuk format seperti laporan laba/rugi yang Anda kirim, isi data detailnya di sheet **FINANCIALS**.
    """)
...
# =========================
st.markdown("## 3. Workforce Productivity")
...
# FJA ANALYTICS
# =========================
st.markdown("## 4. Functional Job Analysis (FJA)")
...
# =========================
st.markdown("## 5. Department Productivity")
...
# =========================
st.markdown("## 6. Capacity Risk Center")
...
# =========================
st.markdown("## 7. Margin Quality & Project Profitability")
...
# WHAT-IF SIMULATOR
# =========================
st.markdown("## 8. What-if Simulator")
...
# =========================
st.markdown("## 9. Board Decision Center")
...
trend["Operating_Margin"] = trend["Operating_Margin_Pct"].apply(lambda x: operating_margin_quality(x, targets)["state"])
trend["Net_Margin"] = trend["Net_Margin_Pct"].apply(lambda x: net_margin_quality(x, targets)["state"])
...
with tc1:
    fig = px.line(
        ...
        hover_data=["Business_Posture", "Margin_Quality", "Operating_Margin", "Net_Margin", "MCR_Health", "Capacity_Risk", "Revenue_YoY", "RPE_YoY", "MCR_Pct", "Gross_Margin_Pct", "Operating_Margin_Pct", "Net_Margin_Pct"],
    )
...
with tc2:
    status_summary = trend[["Year", "Board_Score", "Board_Status", "Business_Posture", "Margin_Quality", "Operating_Margin", "Net_Margin", "MCR_Health", "Capacity_Risk"]].copy()
...
st.divider()
st.caption("MCR vs Revenue Platform V3 — Board & CFO Edition. Interpretasi V3 membaca Income Statement, Net Profit, Revenue Growth, RPE, MCR, Margin Quality, dan Capacity Risk secara bersama-sama.")
