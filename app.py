...
# ENGINES
# =========================
def business_posture(row, targets, target_framework: Optional[pd.DataFrame] = None):
    rev_g = row.get("Revenue_YoY")
    hc_g = row.get("Headcount_YoY")
    rpe_g = row.get("RPE_YoY")
    mcr = row.get("MCR_Pct")
    tf = target_framework if target_framework is not None else pd.DataFrame()
    rev_min = tf_value(tf, "Revenue Growth", "Strong_Min", get_target(targets, "Revenue_Growth_Min", 0.20))
    rpe_min = tf_value(tf, "RPE Growth", "Strong_Min", get_target(targets, "RPE_Growth_Min", 0.15))
    hc_max = tf_value(tf, "Headcount Growth", "Healthy_Max", get_target(targets, "Headcount_Growth_Max_High_Leverage", 0.10))
    mcr_watch = tf_value(tf, "MCR", "Watch_Max", get_target(targets, "MCR_Watch_Max", 0.09))
...
def margin_quality(gm, targets, target_framework: Optional[pd.DataFrame] = None):
    tf = target_framework if target_framework is not None else pd.DataFrame()
    strong = tf_value(tf, "Gross Margin", "Strong_Min", get_target(targets, "Gross_Margin_Strong_Min", 0.30))
    healthy = tf_value(tf, "Gross Margin", "Healthy_Min", get_target(targets, "Gross_Margin_Healthy_Min", 0.20))
    watch = tf_value(tf, "Gross Margin", "Watch_Min", get_target(targets, "Gross_Margin_Watch_Min", 0.15))
    pressure = tf_value(tf, "Gross Margin", "Pressure_Min", get_target(targets, "Gross_Margin_Pressure_Min", 0.10))
    if pd.isna(gm):
...
def mcr_health(mcr, targets, target_framework: Optional[pd.DataFrame] = None):
    tf = target_framework if target_framework is not None else pd.DataFrame()
    ultra = tf_value(tf, "MCR", "Healthy_Min", get_target(targets, "MCR_Ultra_Efficiency_Max", 0.05))
    healthy = tf_value(tf, "MCR", "Healthy_Max", get_target(targets, "MCR_Healthy_Max", 0.07))
    watch = tf_value(tf, "MCR", "Watch_Max", get_target(targets, "MCR_Watch_Max", 0.09))
    pressure = tf_value(tf, "MCR", "Pressure_Max", get_target(targets, "MCR_Cost_Pressure_Max", 0.14))
    if pd.isna(mcr):
...
def hc_guardrails(row, targets):
    ...
    target = tf_value(target_framework if "target_framework" in globals() else pd.DataFrame(), "MCR", "Healthy_Max", get_target(targets, "MCR_Healthy_Max", 0.07))
...
def load_target_framework(sheets: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """Read KPI target framework from Excel. Priority: TARGET_FRAMEWORK, then BENCHMARK_FRAMEWORK."""
    source = None
    for sh in ["TARGET_FRAMEWORK", "BENCHMARK_FRAMEWORK"]:
        if sh in sheets and sheets[sh] is not None and not sheets[sh].empty:
            source = sheets[sh].copy()
            break
    if source is None or source.empty:
        return pd.DataFrame()

    # Normalize columns from either old or new format.
    source.columns = [str(c).strip() for c in source.columns]
    # Remove decorative rows if any.
    if "KPI" not in source.columns:
        # Try to detect header row containing KPI.
        for idx, row in source.iterrows():
            vals = [str(v).strip() for v in row.tolist()]
            if "KPI" in vals:
                new_cols = vals
                source = source.iloc[idx + 1:].copy()
    ...
    # Canonical target framework columns.
    rename_map = {
        "Framework Target": "Target_Framework",
        "Target Framework": "Target_Framework",
        "Benchmark Basis": "Benchmark_Basis",
        "Why It Matters": "Why_It_Matters",
        "Healthy Min": "Healthy_Min",
        "Healthy Max": "Healthy_Max",
        "Watch Min": "Watch_Min",
        "Watch Max": "Watch_Max",
        "Pressure Min": "Pressure_Min",
        "Pressure Max": "Pressure_Max",
        "Critical Min": "Critical_Min",
        "Critical Max": "Critical_Max",
        "Strong Min": "Strong_Min",
        "Strong Max": "Strong_Max",
        "Direction": "Direction",
        "Unit": "Unit",
    ...
    source = source.rename(columns={k: v for k, v in rename_map.items() if k in source.columns})
    source["KPI"] = source["KPI"].astype(str).str.strip()
    source = source[source["KPI"].ne("") & source["KPI"].str.upper().ne("NAN")].copy()

    numeric_cols = [
        "Strong_Min", "Strong_Max", "Healthy_Min", "Healthy_Max", "Watch_Min", "Watch_Max",
        "Pressure_Min", "Pressure_Max", "Critical_Min", "Critical_Max"
    ]
    for c in numeric_cols:
        if c in source.columns:
            source[c] = pd.to_numeric(source[c], errors="coerce")
    if "Direction" not in source.columns:
        source["Direction"] = "Higher Better"
    if "Unit" not in source.columns:
        source["Unit"] = "%"
    return source


def tf_row(tf: pd.DataFrame, kpi: str) -> Optional[pd.Series]:
    if tf is None or tf.empty or "KPI" not in tf.columns:
        return None
    key = kpi.strip().lower()
    found = tf[tf["KPI"].astype(str).str.strip().str.lower().eq(key)]
    if found.empty:
        return None
    return found.iloc[0]


def tf_value(tf: pd.DataFrame, kpi: str, col: str, default: float) -> float:
    r = tf_row(tf, kpi)
    if r is None or col not in r.index or pd.isna(r.get(col)):
        return default
    try:
...
def pct_range_text(low, high, suffix=""):
    if pd.notna(low) and pd.notna(high):
        return f"{low*100:.0f}–{high*100:.0f}%{suffix}"
    if pd.notna(low):
        return f">={low*100:.0f}%{suffix}"
    if pd.notna(high):
        return f"<={high*100:.0f}%{suffix}"
    return "-"


def framework_target_text(tf: pd.DataFrame, kpi: str, fallback: str) -> str:
    r = tf_row(tf, kpi)
    if r is None:
        return fallback
    if "Target_Framework" in r.index and pd.notna(r.get("Target_Framework")) and str(r.get("Target_Framework")).strip():
        return str(r.get("Target_Framework")).strip()
    strong_min = r.get("Strong_Min") if "Strong_Min" in r.index else np.nan
    healthy_min = r.get("Healthy_Min") if "Healthy_Min" in r.index else np.nan
    healthy_max = r.get("Healthy_Max") if "Healthy_Max" in r.index else np.nan
    direction = str(r.get("Direction", "Higher Better"))
    if "Lower" in direction:
        return f"{pct_range_text(healthy_min, healthy_max)} healthy"
    if pd.notna(strong_min):
        return f"{pct_range_text(healthy_min, healthy_max)} healthy; >={strong_min*100:.0f}% strong"
    return f"{pct_range_text(healthy_min, healthy_max)} healthy"


def evaluate_kpi_from_framework(value, tf: pd.DataFrame, kpi: str, fallback_status: str = "-") -> str:
    """Generic status evaluator for display only. It does not override existing business engines unless used explicitly."""
    r = tf_row(tf, kpi)
    if r is None or pd.isna(value):
    ...
    healthy_min = r.get("Healthy_Min") if "Healthy_Min" in r.index else np.nan
    healthy_max = r.get("Healthy_Max") if "Healthy_Max" in r.index else np.nan
    watch_min = r.get("Watch_Min") if "Watch_Min" in r.index else np.nan
    watch_max = r.get("Watch_Max") if "Watch_Max" in r.index else np.nan
    pressure_min = r.get("Pressure_Min") if "Pressure_Min" in r.index else np.nan
    pressure_max = r.get("Pressure_Max") if "Pressure_Max" in r.index else np.nan
    critical_min = r.get("Critical_Min") if "Critical_Min" in r.index else np.nan
    critical_max = r.get("Critical_Max") if "Critical_Max" in r.index else np.nan

    if "Lower" in direction:
        if pd.notna(critical_min) and val >= critical_min:
            return "CRITICAL"
        if pd.notna(pressure_min) and val >= pressure_min:
            return "PRESSURE"
        if pd.notna(watch_max) and val > watch_max:
            return "WATCH"
        if pd.notna(healthy_min) and pd.notna(healthy_max) and healthy_min <= val <= healthy_max:
            return "HEALTHY"
        if pd.notna(healthy_min) and val < healthy_min:
            return "ULTRA_EFFICIENCY"
        return fallback_status

    # Higher is better
    if pd.notna(strong_min) and val >= strong_min:
        return "STRONG"
    if pd.notna(healthy_min) and (pd.isna(healthy_max) or val >= healthy_min):
        if pd.isna(healthy_max) or val <= healthy_max:
            return "HEALTHY"
    if pd.notna(watch_min) and val >= watch_min:
        return "WATCH"
    if pd.notna(pressure_min) and val >= pressure_min:
        return "PRESSURE"
    if pd.notna(critical_max) and val < critical_max:
        return "CRITICAL"
    return fallback_status



def framework_from_targets(targets_df, targets, target_framework: Optional[pd.DataFrame] = None):
    if target_framework is not None and not target_framework.empty:
        tf = target_framework.copy()
        # Build display rows from Excel-driven TARGET_FRAMEWORK.
        display = pd.DataFrame()
        display["KPI"] = tf["KPI"]
        display["Target Framework"] = [
            framework_target_text(tf, str(k), "-") for k in tf["KPI"].tolist()
        ]
        display["Benchmark Basis"] = tf["Benchmark_Basis"] if "Benchmark_Basis" in tf.columns else ""
        display["Why It Matters"] = tf["Why_It_Matters"] if "Why_It_Matters" in tf.columns else ""
        display["Confidence"] = tf["Confidence"] if "Confidence" in tf.columns else "Medium"
        return display
...
def get_presentation_scale(mode: str):
    if mode == "Smartphone":
        return {"global_px": 14.5, "h1_rem": 1.70, "h2_rem": 1.45, "h3_rem": 1.22, "metric_label_rem": 0.92, "metric_value_rem": 1.65, "delta_rem": 0.86, "tab_px": 13, "sidebar_px": 13, "plot_font": 12, "plot_title": 15, "axis_title": 13, "axis_tick": 11, "legend": 11, "bubble_size": 50, "capacity_bubble": 42, "gauge_height": 230, "chart_height": 380, "how_read_px": 13, "container_padding": "0.65rem", "max_width": "100%", "mobile": 1}
    if mode == "Compact":
        return {"global_px": 15.0, "h1_rem": 2.20, "h2_rem": 1.78, "h3_rem": 1.34, "metric_label_rem": 1.00, "metric_value_rem": 1.95, "delta_rem": 0.94, "tab_px": 14, "sidebar_px": 14, "plot_font": 13, "plot_title": 18, "axis_title": 15, "axis_tick": 13, "legend": 13, "bubble_size": 64, "capacity_bubble": 54, "gauge_height": 280, "chart_height": 470, "how_read_px": 14, "container_padding": "1.25rem", "max_width": "1540px", "mobile": 0}
    if mode == "Boardroom":
        return {"global_px": 18.0, "h1_rem": 2.85, "h2_rem": 2.28, "h3_rem": 1.78, "metric_label_rem": 1.25, "metric_value_rem": 2.75, "delta_rem": 1.18, "tab_px": 17, "sidebar_px": 17, "plot_font": 16, "plot_title": 22, "axis_title": 19, "axis_tick": 16, "legend": 16, "bubble_size": 92, "capacity_bubble": 76, "gauge_height": 350, "chart_height": 560, "how_read_px": 16.5, "container_padding": "2.4rem", "max_width": "1560px", "mobile": 0}
    return {"global_px": 16.5, "h1_rem": 2.38, "h2_rem": 1.98, "h3_rem": 1.52, "metric_label_rem": 1.10, "metric_value_rem": 2.18, "delta_rem": 1.03, "tab_px": 15, "sidebar_px": 15, "plot_font": 14, "plot_title": 20, "axis_title": 17, "axis_tick": 14, "legend": 14, "bubble_size": 78, "capacity_bubble": 64, "gauge_height": 310, "chart_height": 520, "how_read_px": 15.5, "container_padding": "2.4rem", "max_width": "1560px", "mobile": 0}

def inject_presentation_mode_css(scale):
    st.markdown(f"""
    <style>
    ...
    div[data-testid="stMetricLabel"] {{ font-size: {scale['metric_label_rem']}rem !important; font-weight: 750 !important; }}
    div[data-testid="stMetricValue"] {{ font-size: {scale['metric_value_rem']}rem !important; font-weight: 850 !important; }}
    div[data-testid="stMetricDelta"] {{ font-size: {scale['delta_rem']}rem !important; }}
    .stTabs [data-baseweb="tab"] {{ font-size: {scale['tab_px']}px !important; font-weight: 800 !important; }}
    [data-testid="stSidebar"] label, [data-testid="stSidebar"] p, [data-testid="stSidebar"] span {{ font-size: {scale['sidebar_px']}px !important; }}
    @media (max-width: 768px) {{
        .main .block-container {{ padding-left: 0.65rem !important; padding-right: 0.65rem !important; padding-top: 0.6rem !important; }}
        .stTabs [data-baseweb="tab-list"] {{ overflow-x: auto !important; flex-wrap: nowrap !important; white-space: nowrap !important; }}
        .stTabs [data-baseweb="tab"] {{ font-size: 12.5px !important; min-width: max-content !important; }}
        div[data-testid="column"] {{ width: 100% !important; flex: 1 1 100% !important; min-width: 100% !important; }}
        .sep-feature-band {{ grid-template-columns: 1fr !important; }}
    }}
    </style>
    """, unsafe_allow_html=True)

def apply_plotly_presentation(fig, scale, height_key="chart_height"):
    fig.update_layout(
    ...
    fig.update_xaxes(title_font=dict(size=int(scale.get("axis_title", 17))), tickfont=dict(size=int(scale.get("axis_tick", 14))))
    fig.update_yaxes(title_font=dict(size=int(scale.get("axis_title", 17))), tickfont=dict(size=int(scale.get("axis_tick", 14))))
    return fig
...
target_framework = load_target_framework(sheets)
...
bp = business_posture(latest, targets, target_framework)
mq = margin_quality(latest.get("Gross_Margin_Pct"), targets, target_framework)
mh = mcr_health(latest.get("MCR_Pct"), targets, target_framework)
...
with boards[1]:
    ...
    if pp_year.empty:
        ...
        with p2:
            ...
            fig.add_hline(y=tf_value(target_framework, "Gross Margin", "Healthy_Min", get_target(targets, "Gross_Margin_Healthy_Min", 0.20)), line_dash="dash", annotation_text="GM Healthy Min")
...
with boards[4]:
    ...
    if pd.notna(scenario_mcr):
        if scenario_mcr <= tf_value(target_framework, "MCR", "Healthy_Max", get_target(targets, "MCR_Healthy_Max", 0.07)) and gross_margin_sim >= tf_value(target_framework, "Gross Margin", "Healthy_Min", get_target(targets, "Gross_Margin_Healthy_Min", 0.20)):
            st.success("Scenario layak: MCR masih dalam target sehat dan Gross Margin memenuhi benchmark.")
        elif scenario_mcr <= tf_value(target_framework, "MCR", "Watch_Max", get_target(targets, "MCR_Watch_Max", 0.09)):
...
with boards[5]:
    ...
    framework = framework_from_targets(sheets.get("TARGETS", pd.DataFrame()), targets, target_framework)
    st.dataframe(framework, hide_index=True, use_container_width=True)

    st.markdown("### Current KPI vs Framework")
    compare = pd.DataFrame([
        {"KPI": "Revenue Growth", "Company": _pct(latest.get("Revenue_YoY")), "Framework": framework_target_text(target_framework, "Revenue Growth", "15–25% healthy; >25% strong"), "Status": evaluate_kpi_from_framework(latest.get("Revenue_YoY"), target_framework, "Revenue Growth", "Strong/Watch based on trend")},
        {"KPI": "RPE Growth", "Company": _pct(latest.get("RPE_YoY")), "Framework": framework_target_text(target_framework, "RPE Growth", ">=15% strong"), "Status": evaluate_kpi_from_framework(latest.get("RPE_YoY"), target_framework, "RPE Growth", "Productivity growth")},
        {"KPI": "MCR", "Company": _pct(latest.get("MCR_Pct")), "Framework": framework_target_text(target_framework, "MCR", "5–7% healthy; 7–9% watch"), "Status": mh["state"]},
        {"KPI": "Gross Margin", "Company": _pct(latest.get("Gross_Margin_Pct")), "Framework": framework_target_text(target_framework, "Gross Margin", "20–30% healthy; >30% strong"), "Status": mq["state"]},
        {"KPI": "Capacity Score", "Company": f"{cr['score']}/100", "Framework": framework_target_text(target_framework, "Capacity Score", "0–30 low; 31–55 medium; 56–75 high; 76–100 critical"), "Status": cr["state"]},
    ])
    st.dataframe(compare, hide_index=True, use_container_width=True)

    st.markdown("### Methodology")
    st.info(
        "Framework ini dibaca dari sheet TARGET_FRAMEWORK di Excel. Ini adalah management benchmark, bukan standar regulasi. Target disusun dari kombinasi praktik industri telekomunikasi dan IT System Integrator, analisis laporan tahunan perusahaan sejenis, praktik workforce planning, serta historical performance perusahaan. Review disarankan minimal triwulanan."
    )
