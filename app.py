...
DEFAULT_FILE_CANDIDATES = [
    os.path.join("data", "SEP_V4_Final_Executive_Intelligence_Template.xlsx"),
    os.path.join("data", "SEP_V4_Final_Project_Productivity_Template.xlsx"),
    "SEP_V4_Final_Project_Productivity_Template.xlsx",
]


def get_default_file() -> str:
    """Return the first bundled template found, for demo/fallback use only."""
    return next((path for path in DEFAULT_FILE_CANDIDATES if os.path.exists(path)), DEFAULT_FILE_CANDIDATES[0])
...
def get_source_metadata(uploaded_file, default_path: str) -> Dict[str, str]:
    """Create a clear provenance label so demo, uploaded, and projected data are never confused."""
    if uploaded_file is not None:
        uploaded_at = datetime.now().strftime("%d %b %Y, %H:%M")
        return {
            "type": "ACTUAL UPLOADED DATA",
            "tone": "success",
            "file_name": uploaded_file.name,
            "detail": f"Diunggah pada {uploaded_at}. Data berlaku untuk sesi ini dan tidak disimpan permanen oleh aplikasi.",
        }
    return {
        "type": "DEMO / TEMPLATE DATA",
        "tone": "warning",
        "file_name": os.path.basename(default_path),
        "detail": "Gunakan Upload Excel untuk melihat data aktual terbaru. Template hanya untuk demo dan validasi struktur.",
    }


def render_data_status(source: Dict[str, str]):
    tone = "#15803d" if source["tone"] == "success" else "#b45309"
    background = "#f0fdf4" if source["tone"] == "success" else "#fffbeb"
    border = "#bbf7d0" if source["tone"] == "success" else "#fde68a"
    st.markdown(
        f"""
        <div style="border:1px solid {border};border-left:5px solid {tone};border-radius:12px;
                    background:{background};padding:12px 15px;margin:4px 0 18px 0;">
            <div style="font-size:12px;font-weight:850;letter-spacing:.06em;color:{tone};">{source['type']}</div>
            <div style="font-size:14px;font-weight:700;color:#172033;margin-top:3px;">{source['file_name']}</div>
            <div style="font-size:12px;color:#526174;margin-top:4px;">{source['detail']}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def build_executive_export(latest: pd.Series, period: Dict, source: Dict[str, str], bp: Dict, mq: Dict, mh: Dict, cr: Dict, bs: Dict, actions: List[str], currency: str) -> bytes:
    """Generate a compact, board-friendly Excel report for the active view."""
    summary = pd.DataFrame([
        ["Report title", "SEP V4 — Executive Snapshot"],
        ["Generated at", datetime.now().strftime("%d %b %Y, %H:%M")],
        ["Data status", source["type"]],
        ["Data file", source["file_name"]],
        ["Period", period["Period_Label"]],
        ["Analysis confidence", period["Data_Confidence"]],
        ["Company health", f"{bs['label']} ({bs['score']}/100)"],
        ["Business posture", bp["state"]],
        ["MCR health", mh["state"]],
        ["Gross margin", mq["state"]],
        ["Capacity risk", f"{cr['state']} ({cr['score']}/100)"],
    ], columns=["Item", "Value"])
    kpis = pd.DataFrame([
        ["Revenue", _money(latest.get("Total_Revenue"), currency)],
        ["Headcount", _num(latest.get("Total_Headcount"))],
        ["Revenue per Employee (RPE)", _money(latest.get("RPE"), currency)],
        ["Manpower Cost Ratio (MCR)", _pct(latest.get("MCR_Pct"))],
        ["Gross Margin", _pct(latest.get("Gross_Margin_Pct"))],
        ["Manpower Cost", _money(latest.get("Total_Manpower_Cost"), currency)],
    ], columns=["KPI", "Active Period"])
    action_plan = pd.DataFrame({"Priority action (next 90 days)": actions[:5]})
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        summary.to_excel(writer, index=False, sheet_name="Executive Snapshot")
        kpis.to_excel(writer, index=False, sheet_name="Executive Snapshot", startrow=len(summary) + 3)
        action_plan.to_excel(writer, index=False, sheet_name="90-Day Action Plan")
        for worksheet in writer.book.worksheets:
            ...
            for cell in worksheet[1]:
                ...
                header_fill.fgColor.rgb = "00351C75"
                header_fill.fill_type = "solid"
                cell.fill = header_fill
            for column_cells in worksheet.columns:
                width = min(max(len(str(cell.value or "")) for cell in column_cells) + 3, 72)
...
with st.sidebar:
    ...
    st.caption("Upload file Excel terbaru untuk mengganti template demo pada sesi ini.")
    uploaded = st.file_uploader("Upload Excel data aktual", type=["xlsx"], help="Gunakan template SEP agar seluruh sheet dan kolom dapat terbaca.")
    default_file = get_default_file()
    if uploaded is not None:
        st.success(f"Data aktual aktif: {uploaded.name}")
    elif os.path.exists(default_file):
        st.warning("Mode demo aktif — upload data aktual sebelum digunakan sebagai dasar keputusan.")
        with open(default_file, "rb") as template_file:
            st.download_button(
                "⬇️ Download template Excel",
                data=template_file.read(),
                file_name=os.path.basename(default_file),
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )
    else:
        st.error("Template default tidak ditemukan. Upload file Excel untuk melanjutkan.")

file_bytes = uploaded.getvalue() if uploaded is not None else None
sheets, warnings = load_workbook(file_bytes, default_file)
source_metadata = get_source_metadata(uploaded, default_file)
...
render_data_status(source_metadata)
export_bytes = build_executive_export(latest, period, source_metadata, bp, mq, mh, cr, bs, actions, currency)
export_col, export_note = st.columns([1.15, 2.85])
with export_col:
    st.download_button(
        "⬇️ Download Executive Snapshot",
        data=export_bytes,
        file_name=f"SEP_Executive_Snapshot_{year_sel}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
    )
with export_note:
    st.caption("Ekspor berisi status sumber data, KPI aktif, status kesehatan bisnis, dan prioritas aksi 90 hari sesuai filter saat ini.")
