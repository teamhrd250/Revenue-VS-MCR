...
# app.py
# Starcom Executive Platform (SEP) V4 — Board & CFO Edition
# Streamlit dashboard based on Excel data model:
# HR_vs_Revenue_V3_Enterprise_Template.xlsx
    ...
    with col_title:
        st.markdown(
            ...
            <div style="padding:4px 0 10px 0;">
                ...
                <div style="font-size:15px; color:#64748b; margin-top:6px;">Executive Intelligence - MCR, revenue leverage, workforce productivity, FJA, capacity risk, and board decision.</div>
            </div>
            """,
...
def clean_and_calc(sheets: Dict[str, pd.DataFrame], analysis_mode: str = "Actual") -> Dict[str, pd.DataFrame]:
    ...
    # V4 monthly actual engine:
    # In YTD Comparable, if MONTHLY_KPI exists, app uses actual month-level data
    # for every comparable year up to comparable_months (e.g. Jan-Jun 2025 vs Jan-Jun 2026).
    # If MONTHLY_KPI is missing/blank, app falls back to the existing FY proxy logic.
    monthly_ytd_used = False
    monthly_dept_revenue = pd.DataFrame()
    if analysis_mode == "YTD Comparable" and "MONTHLY_KPI" in sheets and not sheets["MONTHLY_KPI"].empty:
        mk = sheets["MONTHLY_KPI"].copy()
        mk.columns = [str(c).strip() for c in mk.columns]
        required_monthly_cols = {"Year", "Month_No"}
        if required_monthly_cols.issubset(set(mk.columns)):
            ...
            for _c in ["Revenue_Recognized", "COGS_Direct", "Avg_Headcount", "New_Hires", "Exits", "Payroll_Gross", "Overtime", "Bonus", "Benefits", "Employer_Tax", "Total_Manpower_Cost"]:
                if _c not in mk.columns:
                    mk[_c] = 0.0
                mk[_c] = pd.to_numeric(mk[_c], errors="coerce").fillna(0.0)

            if "Dept_ID" not in mk.columns:
                mk["Dept_ID"] = ""
            mk["Dept_ID"] = mk["Dept_ID"].astype(str).str.strip()
            mk["Business_Line"] = mk.get("Business_Line", "Total Company")

            # Revenue/COGS can be filled either at company-total level or department level.
            rev_m = mk[(mk["Revenue_Recognized"] != 0) | (mk["COGS_Direct"] != 0)].copy()
            if not rev_m.empty:
                rev = (
                    rev_m.groupby(["Year", "Business_Line"], as_index=False)
                    .agg(Revenue_Recognized=("Revenue_Recognized", "sum"), COGS_Direct=("COGS_Direct", "sum"))
                )
                rev["Gross_Profit"] = rev["Revenue_Recognized"] - rev["COGS_Direct"]
                rev["Gross_Margin_Pct"] = np.where(rev["Revenue_Recognized"] > 0, rev["Gross_Profit"] / rev["Revenue_Recognized"], np.nan)
                monthly_ytd_used = True

                dept_rev_m = rev_m[rev_m["Dept_ID"].ne("") & ~rev_m["Dept_ID"].str.upper().isin(["COMPANY_TOTAL", "TOTAL", "TOTAL COMPANY"])].copy()
                if not dept_rev_m.empty:
                    monthly_dept_revenue = (
                        dept_rev_m.groupby(["Year", "Dept_ID"], as_index=False)
                        .agg(Dept_Revenue=("Revenue_Recognized", "sum"))
                    )

            # Headcount/Payroll rows should have real Dept_ID values.
            dept_m = mk[mk["Dept_ID"].ne("") & ~mk["Dept_ID"].str.upper().isin(["COMPANY_TOTAL", "TOTAL", "TOTAL COMPANY"])].copy()
            if not dept_m.empty and dept_m["Avg_Headcount"].sum() > 0:
                hc = (
                    dept_m.groupby(["Year", "Dept_ID"], as_index=False)
                    .agg(
                ...
                hc["Notes"] = f"Monthly actual YTD {comparable_months}M"

            if not dept_m.empty and dept_m["Total_Manpower_Cost"].sum() > 0:
                pay = (
                    dept_m.groupby(["Year", "Dept_ID"], as_index=False)
                    .agg(
                ...
                pay["Payment_Type"] = "Monthly Actual"
                pay["Notes"] = f"Monthly actual YTD {comparable_months}M"

            mode_note = (
                f"YTD Comparable: memakai MONTHLY_KPI actual Jan-{comparable_months} untuk setiap tahun. "
                f"Jika data bulanan belum lengkap, isi sheet MONTHLY_KPI agar Gross Margin, MCR, RPE, dan seluruh presentation layer berubah berdasarkan realisasi YTD aktual."
            )
    ...
    if analysis_mode in ["Annual Projection", "YTD Comparable", "MCR Focus"] and not monthly_ytd_used:
    ...
    if analysis_mode in ["Annual Projection", "YTD Comparable"] and not monthly_ytd_used:
    ...
    return {
        ...
        "monthly_ytd_used": monthly_ytd_used,
    }
...
if analysis_mode == "YTD Comparable" and data.get("monthly_ytd_used", False):
    st.success("MONTHLY_KPI aktif: Gross Margin, MCR, RPE, Board Score, Hiring Decision, CEO One-Page, dan Executive Closing dihitung dari realisasi YTD aktual per bulan.")
elif analysis_mode == "YTD Comparable":
    st.warning("MONTHLY_KPI belum aktif/terisi. Dashboard memakai proxy FY x bulan YTD. Untuk komparasi aktual Jan-Jun vs Jan-Jun, isi sheet MONTHLY_KPI.")
...
with st.expander("How to Read Executive Summary"):
    ...
    - **YTD Comparable** = jika `MONTHLY_KPI` terisi, dashboard membandingkan realisasi aktual bulan yang sama, misalnya Jan-Jun 2025 vs Jan-Jun 2026. Jika belum terisi, dashboard memakai proxy FY x bulan YTD.
...
st.divider()
st.caption("Starcom Executive Platform (SEP) V4 — Board & CFO Edition. Interpretasi MCR harus selalu dibaca bersama Revenue Growth, RPE, Headcount, Margin Quality, dan Capacity Risk.")
