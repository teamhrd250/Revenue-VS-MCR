...
def clean_and_calc(sheets: Dict[str, pd.DataFrame], analysis_mode: str = "Actual") -> Dict[str, pd.DataFrame]:
    ...
    # Financials / Income Statement view
    if "FINANCIALS" in sheets and not sheets["FINANCIALS"].empty:
        ...
        financial_cols = [
            "Revenue", "COGS", "Admin_OPEX_Non_Manpower", "Selling_OPEX_Non_Manpower",
            "Other_Operating_Cost", "Interest_Expense", "Tax_Expense"
        ]
        ...
        if analysis_mode == "Annualized":
            ...
            for _c in financial_cols:
                fin[_c] = fin[_c] * _fin_factor

        fin["Financial_Revenue"] = fin["Revenue"]
        fin["Financial_COGS"] = fin["COGS"]
        fin["Gross_Profit_Financial"] = fin["Revenue"] - fin["COGS"]
        fin["Operating_OPEX_Non_Manpower"] = (
            fin["Admin_OPEX_Non_Manpower"] + fin["Selling_OPEX_Non_Manpower"] + fin["Other_Operating_Cost"]
        )
        # EBIT excludes manpower cost by merging payroll total below. This assumes FINANCIALS OPEX is non-manpower.
        fin = fin.merge(pay_tot, on="Year", how="left")
        fin["Total_Manpower_Cost"] = fin["Total_Manpower_Cost"].fillna(0.0)
        fin["EBIT"] = fin["Gross_Profit_Financial"] - fin["Total_Manpower_Cost"] - fin["Operating_OPEX_Non_Manpower"]
        fin["PBT"] = fin["EBIT"] - fin["Interest_Expense"]
        fin["Net_Profit"] = fin["PBT"] - fin["Tax_Expense"]
        fin["Operating_Margin_Pct"] = np.where(fin["Revenue"] > 0, fin["EBIT"] / fin["Revenue"], np.nan)
        fin["Net_Margin_Pct"] = np.where(fin["Revenue"] > 0, fin["Net_Profit"] / fin["Revenue"], np.nan)
        fin["Admin_OPEX_Ratio"] = np.where(fin["Revenue"] > 0, fin["Admin_OPEX_Non_Manpower"] / fin["Revenue"], np.nan)
        fin["Interest_Ratio"] = np.where(fin["Revenue"] > 0, fin["Interest_Expense"] / fin["Revenue"], np.nan)
        fin["Tax_Ratio"] = np.where(fin["Revenue"] > 0, fin["Tax_Expense"] / fin["Revenue"], np.nan)
        fin_tot = fin[[
            "Year", "Financial_Revenue", "Financial_COGS", "Admin_OPEX_Non_Manpower",
            "Selling_OPEX_Non_Manpower", "Other_Operating_Cost", "Operating_OPEX_Non_Manpower",
            "Interest_Expense", "Tax_Expense", "EBIT", "PBT", "Net_Profit",
            "Operating_Margin_Pct", "Net_Margin_Pct", "Admin_OPEX_Ratio", "Interest_Ratio", "Tax_Ratio"
        ]].copy()
    else:
        fin_tot = yr[["Year", "Total_Revenue", "Total_COGS", "Total_Manpower_Cost"]].copy()
        fin_tot["Financial_Revenue"] = fin_tot["Total_Revenue"]
        fin_tot["Financial_COGS"] = fin_tot["Total_COGS"]
        fin_tot["Admin_OPEX_Non_Manpower"] = 0.0
        fin_tot["Selling_OPEX_Non_Manpower"] = 0.0
        fin_tot["Other_Operating_Cost"] = 0.0
        fin_tot["Operating_OPEX_Non_Manpower"] = 0.0
        fin_tot["Interest_Expense"] = 0.0
        fin_tot["Tax_Expense"] = 0.0
        fin_tot["EBIT"] = fin_tot["Total_Revenue"] - fin_tot["Total_COGS"] - fin_tot["Total_Manpower_Cost"]
        fin_tot["PBT"] = fin_tot["EBIT"]
        fin_tot["Net_Profit"] = fin_tot["PBT"]
        fin_tot["Operating_Margin_Pct"] = np.where(fin_tot["Financial_Revenue"] > 0, fin_tot["EBIT"] / fin_tot["Financial_Revenue"], np.nan)
        fin_tot["Net_Margin_Pct"] = np.where(fin_tot["Financial_Revenue"] > 0, fin_tot["Net_Profit"] / fin_tot["Financial_Revenue"], np.nan)
        fin_tot["Admin_OPEX_Ratio"] = 0.0
        fin_tot["Interest_Ratio"] = 0.0
        fin_tot["Tax_Ratio"] = 0.0

    # Merge financial metrics into yearly table
    yr = yr.merge(
        fin_tot[[
            "Year", "Admin_OPEX_Non_Manpower", "Selling_OPEX_Non_Manpower", "Other_Operating_Cost",
            "Operating_OPEX_Non_Manpower", "Interest_Expense", "Tax_Expense", "EBIT", "PBT", "Net_Profit",
            "Operating_Margin_Pct", "Net_Margin_Pct", "Admin_OPEX_Ratio", "Interest_Ratio", "Tax_Ratio"
        ]],
        on="Year",
        how="left",
...
def operating_margin_quality(op_margin, targets: Dict[str, float]) -> Dict[str, str]:
    strong = get_target(targets, "Operating_Margin_Strong_Min", 0.20)
    healthy = get_target(targets, "Operating_Margin_Healthy_Min", 0.15)
    watch = get_target(targets, "Operating_Margin_Watch_Min", 0.10)
    pressure = get_target(targets, "Operating_Margin_Pressure_Min", 0.05)
    if pd.isna(op_margin):
        return {"state": "UNKNOWN", "tone": "info", "hint": "Operating Margin belum tersedia."}
    if op_margin >= strong:
        return {"state": "STRONG", "tone": "success", "hint": "Operating profit sangat kuat setelah biaya operasional."}
    if op_margin >= healthy:
        return {"state": "HEALTHY", "tone": "success", "hint": "Operating profit sehat; biaya operasional masih terkendali."}
    if op_margin >= watch:
        return {"state": "WATCH", "tone": "warning", "hint": "Operating Margin perlu dipantau; cek admin/selling/operating cost."}
    if op_margin >= pressure:
        return {"state": "PRESSURE", "tone": "warning", "hint": "Operating profit mulai tertekan oleh biaya operasional."}
    return {"state": "CRITICAL", "tone": "error", "hint": "Operating Margin kritikal; perlu cost recovery dan review struktur biaya."}


def net_margin_quality(net_margin, targets: Dict[str, float]) -> Dict[str, str]:
    strong = get_target(targets, "Net_Margin_Strong_Min", 0.12)
    healthy = get_target(targets, "Net_Margin_Healthy_Min", 0.08)
    watch = get_target(targets, "Net_Margin_Watch_Min", 0.04)
    pressure = get_target(targets, "Net_Margin_Pressure_Min", 0.02)
    if pd.isna(net_margin):
        return {"state": "UNKNOWN", "tone": "info", "hint": "Net Margin belum tersedia."}
    if net_margin >= strong:
        return {"state": "STRONG", "tone": "success", "hint": "Net profit sangat kuat setelah bunga dan pajak."}
    if net_margin >= healthy:
        return {"state": "HEALTHY", "tone": "success", "hint": "Net profit sehat dan masih layak untuk ekspansi terukur."}
    if net_margin >= watch:
        return {"state": "WATCH", "tone": "warning", "hint": "Net Margin perlu dipantau; laba bersih mulai tipis."}
    if net_margin >= pressure:
        return {"state": "PRESSURE", "tone": "warning", "hint": "Net profit mulai tertekan; cek interest, tax, dan OPEX."}
    return {"state": "CRITICAL", "tone": "error", "hint": "Net Margin kritikal; revenue belum cukup menjadi laba bersih."}
...
def board_score_engine(row: pd.Series, targets: Dict[str, float], cap_state: str = "LOW") -> Dict[str, object]:
    """Board Score Engine v4 with Financial Quality.

    Weighted score:
    - Revenue Growth 20%
    - RPE Growth 15%
    - Gross Margin Quality 15%
    - MCR Health 15%
    - Operating Margin 15%
    - Net Margin 10%
    - Capacity Risk 10%
...
with f1:
    st.metric("🏢 Admin OPEX", _money(latest.get("Admin_OPEX_Non_Manpower"), currency), _pct(latest.get("Admin_OPEX_Ratio")))
with f2:
    st.metric("📉 EBIT", _money(latest.get("EBIT"), currency), _pct(latest.get("Operating_Margin_Pct")))
with f3:
    st.metric("🏦 Interest + Tax", _money((latest.get("Interest_Expense") or 0) + (latest.get("Tax_Expense") or 0), currency))
with f4:
...
memo = (
    ...
    f"Business Posture terbaca **{bp['state']}**, Margin Quality **{mq['state']}**, MCR Health **{mh['state']}**, "
    f"Operating Margin **{opq['state']}**, dan Net Margin **{netq['state']}**. "
    f"EBIT terbaca **{_money(latest.get('EBIT'), currency)}** dan Net Profit **{_money(latest.get('Net_Profit'), currency)}**. "
...
with st.expander("How to Read Executive Summary"):
    ...
    | Operating Margin Healthy | **>=15%** | EBIT margin sehat setelah biaya operasional |
    | Net Margin Healthy | **>=8%** | Laba bersih sehat setelah bunga dan pajak |
    | Admin OPEX Ratio Max | **<=10%** | Biaya administrasi non-manpower terkendali |
    | Interest Ratio Max | **<=3%** | Beban bunga masih terkendali terhadap revenue |
    ...
    - Jika **Gross Margin sehat tetapi Net Margin rendah**, masalahnya biasanya ada di OPEX, bunga pinjaman, atau pajak.
    - FINANCIALS harus diisi sebagai biaya **non-manpower** agar tidak double count dengan PAYROLL_YR.
    """)
...
# =========================
st.markdown("## 2. Financial Profitability")

pl1, pl2 = st.columns([1.1, 1.0])
with pl1:
    waterfall = pd.DataFrame({
        "Stage": ["Revenue", "COGS", "Manpower", "Non-Manpower OPEX", "Interest", "Tax", "Net Profit"],
        "Amount": [
            latest.get("Total_Revenue"),
            -latest.get("Total_COGS"),
            -latest.get("Total_Manpower_Cost"),
            -latest.get("Operating_OPEX_Non_Manpower"),
            -latest.get("Interest_Expense"),
            -latest.get("Tax_Expense"),
    ...
    fig = go.Figure(go.Waterfall(
        ...
        connector={"line": {"color": "rgba(100,116,139,.5)"}},
    ))
    fig.update_layout(title=f"Profit Bridge — {year_sel}", margin=dict(l=10, r=10, t=45, b=10), height=390)
...
with pl2:
    ...
    fig.update_layout(margin=dict(l=10, r=10, t=45, b=10), height=390)
    st.plotly_chart(fig, use_container_width=True)

with st.expander("How to Read Financial Profitability"):
    st.markdown("""
    - **Revenue** menunjukkan pendapatan sebelum biaya.
    - **Gross Profit** = Revenue - COGS.
    - **EBIT / Operating Profit** = Gross Profit - Manpower Cost - Non-Manpower OPEX.
    - **PBT** = EBIT - Interest Expense.
    - **Net Profit** = PBT - Tax Expense.
    - Jika Revenue naik tetapi **Net Profit turun**, masalahnya biasanya bukan sales, tetapi biaya operasional, bunga, pajak, atau margin project.
    - Isi `FINANCIALS` sebagai biaya **non-manpower** agar tidak double count dengan sheet `PAYROLL_YR`.
    """)
